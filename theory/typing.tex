...
\subsection{Auxiliary Functions}
    \begin{itemize}
        ...
        \item \textbf{unify\_local \& unify\_choreo}\\
              ...
              \highlighttexttt{TFuns} type which is used to represent the type of the function.
        \item \textbf{apply\_subst\_typ\_local \& apply\_subst\_typ\_choreo}\\
              ...
              the only difference is that the \highlighttexttt{apply\_subst\_typ\_choreo} can handle the choreographic \highlighttexttt{TFuns} type.
    ...
    \subsubsection{Type Inference for Choreo Expression}\label{choreo_expr}
        ...
        \begin{itemize}
            ...
            \item \textbf{Function Definition} \texttt{Choreo.FunDef (args, e)}\\
                  ...
                  And we use the \texttt{Choreo.TFuns} type to represent the function type, so \texttt{int -> (bool -> (string -> int))}
                  would be represented as \texttt{TFuns (int, TFuns (bool, TFuns (string, int)))}.
            \item \textbf{Function Application} \texttt{Choreo.FunApp (func, param)}\\
                  ...
                  we need to check if the type of the function is a \texttt{TFuns} type, if not we raise an expected function type exception.
