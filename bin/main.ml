...
let usage_msg =
  "USAGE: pirc <file> [-ast-dump <pprint|json|dot>] [-msg-backend \
   <domain|http|mpi>]"
...
let speclist =
  [
    ...
    ( "--ast-dump",
      Arg.Symbol
        ([ "pprint"; "json"; "dot" ], fun s -> ast_dump_format := Some s),
      "Dump the AST in the specified format (pprint, json, dot)" );
    ( "--msg-backend",
      Arg.Symbol ([ "domain"; "http"; "mpi" ], fun s -> msg_backend := s),
      "Choose communication backend (domain: shared memory, http: HTTP, mpi: \
       MPI)" );
    ( "--package",
      Arg.String
        (fun s ->
          check_no_start_end_commas s;
          package_list := s),
    ...
    ( "-w",
      Arg.String
        (fun s ->
          check_no_start_end_commas s;
          ocamlopt_warn_flags := s),
      "Enable/disable ocamlopt warning flags" );
  ]

(* Change the extension of a filename *)
let change_extension filename new_suffix =
  Filename.remove_extension filename ^ new_suffix
...
let () =
  ...
  if !file_ic = None || !input_filename = "" then (
