    ...
    List.concat_map (fun stmt ->
      ...
      | Ast_core.Choreo.M.ImportDecl (filename, _) ->
        ...
        if List.mem full_path seen_files then 
          raise (Import_error ("Transitive cycle detected: " ^ String.concat " -> " (seen_files @ [full_path]))) 
          ...
          (* how main parses a file:
            ...
            resolve_imports_helper (full_path :: seen_files) (Filename.dirname full_path) imported_stmts
