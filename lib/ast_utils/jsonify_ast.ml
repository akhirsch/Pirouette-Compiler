...
let rec jsonify_local_type = function
  ...
  | Local.TProd (t1, t2, _) ->
      `Assoc
        [ ("TProd", `List [ jsonify_local_type t1; jsonify_local_type t2 ]) ]
  | Local.TSum (t1, t2, _) ->
  ...
  | Local.TVariant (constructors, _) ->
    `Assoc
      [ ( "TVariant"
        , `List
            (List.map
               (fun { Local.name; args; typ = TypId (typ_id, _); info = _ } ->
                 `Assoc
                   [ "name", `String name
...
let rec jsonify_local_expr = function
  ...
  | Local.Pair (e1, e2, _) ->
      `Assoc
        [ ("Pair", `List [ jsonify_local_expr e1; jsonify_local_expr e2 ]) ]
  | Local.UnOp (op, e, _) ->
      `Assoc
        [
          ( "UnOp",
            `Assoc
              [ ("op", jsonify_un_op op); ("choreo_e", jsonify_local_expr e) ]
          );
        ]
  ...
  | Local.Construct (name, exprs, TypId (typ_id, _), _) ->
      `Assoc
        [ ( "Construct"
          , `Assoc
              [ "name", `String name
  ...
  | Choreo.TVariant (constructors, _) ->
    `Assoc
      [ ( "TVariant"
        , `List
            (List.map
               (fun { Choreo.name; args; typ = Local.TypId (typ_id, _); info = _ } ->
                 `Assoc
                   [ "name", `String name
...
let rec jsonify_choreo_pattern = function
  ...
  | Choreo.Pair (p1, p2, _) ->
      `Assoc
        [
          ( "Pair",
            `List [ jsonify_choreo_pattern p1; jsonify_choreo_pattern p2 ] );
        ]
  | Choreo.LocPat (LocId (loc, _), p, _) ->
    `Assoc
      [ "LocPat", `Assoc [ "loc", `String loc; "local_patt", jsonify_local_pattern p ] ]
  | Choreo.PConstruct (name, patterns, Local.TypId (typ_id, _),  _) ->
    `Assoc
      [ ( "PConstruct"
        , `Assoc
            [ "name", `String name
  ...
  | Choreo.TypeDecl (TypId (id, _), t, _) ->
      `Assoc
        [
          ( "TypeDecl",
            `Assoc
              [ ("id", `String id); ("choreo_type", jsonify_choreo_type t) ] );
        ]
...
and jsonify_choreo_expr = function
  ...
  | Choreo.Pair (e1, e2, _) ->
      `Assoc
        [ ("Pair", `List [ jsonify_choreo_expr e1; jsonify_choreo_expr e2 ]) ]
  ...
  | Choreo.Construct (name, exprs, Local.TypId (typ_id, _), _) ->
    `Assoc
      [ ( "Construct"
        , `Assoc
            [ "name", `String name
  ...
  | Net.TVariant (constructors, _) ->
    `Assoc
      [ ( "TVariant"
        , `List
            (List.map
               (fun { Net.name; args; typ =_; info = _ } -> (* had to add typ but it shouldn't... fix laterrrrrrr*)
                 `Assoc
                   [ "name", `String name
...
let rec jsonify_net_stmt = function
      ...
      `Assoc
        [
          ( "Decl",
            `Assoc
              [
                ("net_pattern", jsonify_local_pattern p);
  ...
  | Net.TypeDecl (TypId (id, _), t, _) ->
      `Assoc
        [
          ( "TypeDecl",
            `Assoc [ ("id", `String id); ("net_type", jsonify_net_type t) ] );
        ]
...
and jsonify_net_expr = function
  ...
  | Net.Send (e, LocId (loc, _), _) ->
      `Assoc
        [
          ( "Send",
            `Assoc [ ("net_expr", jsonify_net_expr e); ("to", `String loc) ] );
        ]
  ...
  | Net.FunApp (e1, e2, _) ->
      `Assoc
        [
          ( "FunApp",
            `Assoc
              [ ("fun", jsonify_net_expr e1); ("arg", jsonify_net_expr e2) ] );
        ]
  | Net.Pair (e1, e2, _) ->
  ...
  | Net.Construct (name, exprs, Local.TypId (typ_id, _), _) ->
      ...
      [ ( "Construct"
        , `Assoc
            [ "name", `String name; "exprs", `List (List.map jsonify_net_expr exprs); "typ", `String typ_id ]
        )
      ]
;;
