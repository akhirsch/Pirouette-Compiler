(** Choreography visualization using Graphviz DOT language.
    ...
    This module provides utilities for visualizing Pirouette choreographies
    as directed graphs using the DOT graph description language. The generated
    DOT code can be rendered into images using Graphviz tools.
    
    {2 Visualization Features}
    
    - Nodes represent choreographic statements and expressions
    - Edges show control flow and communication patterns
    - Support for all choreographic constructs (send, select, if/then/else, etc.)
    - Customizable node labels via metadata conversion
    
    {2 Prerequisites}
    
    Before using this module, you need:
    1. Graphviz installed on your system
...
(** [generate_dot_code metadata_to_string stmts] generates DOT graph code for
    ...
    {b Parameters:}
    - [metadata_to_string]: Function to convert AST metadata to strings (for node labels)
    - [stmts]: The choreographic statement block to visualize
    
    {b Returns} a complete DOT graph specification as a string, ready to be processed
    by Graphviz tools (dot, neato, etc.) to generate visual diagrams.
