...
let ast_local_value_info_map : ('a -> 'b) -> 'a Ast_core.Local.M.value -> 'b Ast_core.Local.M.value = 
  fun map -> function
    | Int(i, metadata) -> Int(i, map metadata)
    | String (str, metadata) -> String(str, map metadata)
    | Bool (b, metadata) -> Bool(b, map metadata)

let ast_local_unop_info_map : ('a -> 'b) -> 'a Ast_core.Local.M.un_op -> 'b Ast_core.Local.M.un_op = 
  fun map -> function
    | Not (metadata) -> Not (map metadata)
    | Neg (metadata) -> Neg (map metadata)

let ast_local_binop_info_map : ('a -> 'b) -> 'a Ast_core.Local.M.bin_op -> 'b Ast_core.Local.M.bin_op = 
  fun map -> function
...
let rec ast_local_pattern_info_map : ('a -> 'b) -> 'a Ast_core.Local.M.pattern -> 'b Ast_core.Local.M.pattern = 
  fun map -> function
  | Default metadata -> Default (map metadata)
  | Val (value, metadata) -> Val (ast_local_value_info_map map value, map metadata)
  | Var (VarId (var_name, var_metadata), metadata) -> Var (VarId (var_name, map var_metadata), map metadata)
  | Pair (pattern1, pattern2, metadata) -> Pair (ast_local_pattern_info_map map pattern1, ast_local_pattern_info_map map pattern2, map metadata)
  | Left (pattern, metadata) -> Left (ast_local_pattern_info_map map pattern, map metadata)
  | Right (pattern, metadata) -> Right (ast_local_pattern_info_map map pattern, map metadata)
;;

let ast_local_loc_id : ('a -> 'b) -> 'a Ast_core.Local.M.loc_id -> 'b Ast_core.Local.M.loc_id = 
  fun map -> function
  | LocId (local_id_name, local_id_metadata) -> LocId (local_id_name, map local_id_metadata)

let rec ast_local_type_info_map : ('a -> 'b) -> 'a Ast_core.Local.M.typ -> 'b Ast_core.Local.M.typ = 
  fun map -> function
...
let rec info_map_pattern_match : ('a -> 'b) -> ('a Ast_core.Local.M.pattern * 'a Ast_core.Local.M.expr) list -> ('b Ast_core.Local.M.pattern * 'b Ast_core.Local.M.expr) list = 
  fun map -> function 
  | [] -> []
  | (pattern, expr) ::d -> (ast_local_pattern_info_map map pattern, ast_local_expr_info_map map expr) :: (info_map_pattern_match map d)

and ast_local_expr_info_map : ('a -> 'b) -> 'a Ast_core.Local.M.expr -> 'b Ast_core.Local.M.expr = 
  fun map -> function
  | Unit metadata -> Unit (map metadata)
  | Val (value, metadata) -> Val (ast_local_value_info_map map value, map metadata)
  | Var (VarId (var_name, var_metadata), metadata) -> Var (VarId (var_name, map var_metadata), map metadata)
  | UnOp (un_op, expr, metadata) -> UnOp (ast_local_unop_info_map map un_op, ast_local_expr_info_map map expr, map metadata)
  | BinOp (expr, bin_op,expr2, metadata) -> BinOp (ast_local_expr_info_map map expr, ast_local_binop_info_map map bin_op, ast_local_expr_info_map map expr2, map metadata)
  | Let (VarId (var_name, var_metadata), typ, expr1, expr2, metadata) -> Let (VarId (var_name, map var_metadata), ast_local_type_info_map map typ, ast_local_expr_info_map map expr1, ast_local_expr_info_map map expr2, map metadata)
  | Pair (expr1, expr2, metadata) -> Pair (ast_local_expr_info_map map expr1, ast_local_expr_info_map map expr2, map metadata) 
  ...
  | Left (expr, metadata) -> Left (ast_local_expr_info_map map expr, map metadata)
  | Right (expr, metadata) -> Right (ast_local_expr_info_map map expr, map metadata)
  | Match (expr,patterns, metadata) -> Match (ast_local_expr_info_map map expr, info_map_pattern_match map patterns, map metadata)
;;

let rec ast_choreo_type_info_map : ('a -> 'b) -> 'a Ast_core.Choreo.M.typ -> 'b Ast_core.Choreo.M.typ = 
  fun map -> function
  | TUnit metadata ->  TUnit (map metadata)
...
let rec ast_choreo_pattern_info_map : ('a -> 'b) -> 'a Ast_core.Choreo.M.pattern -> 'b Ast_core.Choreo.M.pattern = 
  fun map -> function
  | Default metadata -> Default (map metadata)
  | Var (VarId (name, var_metadata), metadata) -> Var (VarId (name, map var_metadata), map metadata)
  | Pair (pattern1, pattern2, metadata) -> Pair (ast_choreo_pattern_info_map map pattern1, ast_choreo_pattern_info_map map pattern2, map metadata)
  | LocPat (loc_id, local_pattern, metadata) -> LocPat (ast_local_loc_id map loc_id, ast_local_pattern_info_map map local_pattern, map metadata)
  | Left (choreo_pattern, metadata)-> Left (ast_choreo_pattern_info_map map choreo_pattern, map metadata)
  | Right (choreo_pattern,metadata) -> Right (ast_choreo_pattern_info_map map choreo_pattern, map metadata)
;;

let rec ast_choreo_pattern_list_info_map : ('a -> 'b) -> 'a Ast_core.Choreo.M.pattern list -> 'b Ast_core.Choreo.M.pattern list = 
  fun map -> function
  | [] -> []
  | h::d -> (ast_choreo_pattern_info_map map h) :: (ast_choreo_pattern_list_info_map map d)
;;

let rec info_map_pattern_match : ('a -> 'b) -> ('a Ast_core.Choreo.M.pattern * 'a Ast_core.Choreo.M.expr) list -> ('b Ast_core.Choreo.M.pattern * 'b Ast_core.Choreo.M.expr) list = 
  fun map -> function 
  | [] -> []
  | (pattern, expr) ::d -> (ast_choreo_pattern_info_map map pattern, ast_choreo_expr_info_map map expr) :: (info_map_pattern_match map d)

and ast_choreo_expr_info_map : ('a -> 'b) -> 'a Ast_core.Choreo.M.expr -> 'b Ast_core.Choreo.M.expr = 
  fun (map : 'a -> 'b) -> function
  | Unit metadata -> Unit (map metadata)
  | Var (VarId (name, var_metadata), metadata) -> Var (VarId (name, map var_metadata), map metadata)
  | LocExpr (loc_id, local_expr, metadata) -> LocExpr (ast_local_loc_id map loc_id, ast_local_expr_info_map map local_expr, map metadata) 
  | Send (loc_id, expr, loc_id2, metadata) -> Send (ast_local_loc_id map loc_id, ast_choreo_expr_info_map map expr, ast_local_loc_id map loc_id2, map metadata)
  | Sync (loc_id, LabelId (sync_label_name, sync_metadata), loc_id2, expr, metadata) -> Sync (ast_local_loc_id map loc_id, LabelId (sync_label_name, map sync_metadata), ast_local_loc_id map loc_id2, ast_choreo_expr_info_map map expr, map metadata)
  | If (expr1, expr2, expr3, metadata) -> If (ast_choreo_expr_info_map map expr1, ast_choreo_expr_info_map map expr2, ast_choreo_expr_info_map map expr3, map metadata)
  | Let (stmt_block, expr, metadata) -> Let (ast_list_info_map map stmt_block, ast_choreo_expr_info_map map expr, map metadata)
  | FunDef (pattern_list, expr, metadata) -> FunDef (ast_choreo_pattern_list_info_map map pattern_list, ast_choreo_expr_info_map map expr, map metadata)
  | FunApp (expr1, expr2, metadata) -> FunApp (ast_choreo_expr_info_map map expr1, ast_choreo_expr_info_map map expr2, map metadata)
  | Pair (expr1, expr2, metadata) -> Pair (ast_choreo_expr_info_map map expr1, ast_choreo_expr_info_map map expr2, map metadata)
  ...
  | Left (expr, metadata) -> Left (ast_choreo_expr_info_map map expr, map metadata)
  | Right (expr, metadata) -> Right (ast_choreo_expr_info_map map expr, map metadata)
  | Match (expr, patterns, metadata) -> Match (ast_choreo_expr_info_map map expr, info_map_pattern_match map patterns, map metadata)

and ast_info_map : ('a -> 'b) -> 'a Ast_core.Choreo.M.stmt -> 'b Ast_core.Choreo.M.stmt = 
  fun map -> function  
  | Decl (stm_pattern, stmt_type, metadata) -> Decl (ast_choreo_pattern_info_map map stm_pattern, ast_choreo_type_info_map map stmt_type, map metadata)
