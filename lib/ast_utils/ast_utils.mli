(** {b ast_utils} AST utility functions
    ...
    - Traversal and extraction – e.g., [extract_locs] collects all choreography
      locations from a statement block.
    - Serialization and pretty-printing – functions prefixed with [jsonify_*]
      and [pprint_*] convert ASTs to JSON or formatted source text.
    ...
    visualization, and compiler diagnostics), not for end-user programs. They
    help understand how Pirouette code becomes an AST, where you can see the
...
(** [extract_locs] extracts all location identifiers from choreographic
    ...
    Returns a list of unique location names (as strings) found in
    location-qualified types, expressions, or patterns. *)

(** {1 JSON Serialization}

    Functions for converting AST structures to JSON format, useful for external
    tooling, debugging, and data interchange. The JSON output represents the
    internal OCaml AST structure.
...
(** [stringify_jsonify_choreo_ast] converts choreographic statement block
    [stmts] to a JSON string representation.

    Returns the entire AST structure as a JSON-formatted string, including all
    nested expressions, types, and patterns.*)

val stringify_jsonify_net_ast : 'a Ast_core.Net.M.stmt_block -> string
(** [stringify_jsonify_net_ast] converts network statement block [stmts] to a
...
(** [jsonify_choreo_ast] writes choreographic statement block [stmts] as JSON to
    output channel [oc].

    Serializes the AST structure to JSON and writes it directly to the specified
    output channel (file, stdout, etc.).*)

val jsonify_net_ast : out_channel -> 'a Ast_core.Net.M.stmt_block -> unit
(** [jsonify_net_ast] writes network statement block [stmts] as JSON to output
    channel [oc].
...
(** [stringify_pprint_choreo_ast] converts choreographic statement block [stmts]
    to formatted Pirouette source code as a string.
...
(** [stringify_pprint_net_ast] converts network statement block [stmts] to
...
(** [pprint_choreo_ast] writes choreographic statement block [stmts] as
    formatted Pirouette source code to output channel [oc].*)

val pprint_net_ast : out_channel -> 'a Ast_core.Net.M.stmt_block -> unit
(** [pprint_net_ast] writes network statement block [stmts] as formatted source
    code to output channel [oc].

    Outputs the network IR with explicit send/receive operations in a readable
    format. *)

(** {1 Graph Visualization}

    Functions for exporting AST structures to DOT format for visualization with
    Graphviz. Useful for understanding AST structure and debugging
    transformations.
...
val stringify_dot_choreo_ast :
  ('a -> string) -> 'a Ast_core.Choreo.M.stmt_block -> string
(** [stringify_dot_choreo_ast] converts choreographic statement block [stmts] to
    a DOT graph representation as a string.

    Parameters:
    - [meta_to_string]: function to convert metadata ['a] to a string for node
      labels
...
(** [parse_external_name] parses a foreign function name specification into
    [(module_name, function_name, alias)]. See section example for usage. *)
