...
let rec ast_choreo_pattern_list_stringify : 'a Ast_core.Choreo.M.pattern list -> string =
  function
  | [] -> "[]"
  | h :: d ->
    "(("
...
let compile_stdlib () : unit =
  let (path_to_stdlib : string) =
    ...
    | None ->
      prerr_endline
        "Path to Pirouette standard library not set in enviroment variables as \
         \"PIR_STDLIB\"=[ABSOLUTE_PATH_TO_YOUR_STDLIB]\n";
      exit 1
  in
  ...
  let stdlib_ast_file_oc =
    open_out (Filename.dirname path_to_stdlib ^ Filename.dir_sep ^ "stdlib_ast.ml")
  in
