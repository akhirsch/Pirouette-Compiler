(** Code generation for MPI-based communication backend.
    ...
    (Message Passing Interface) for communication. It implements a message-passing
    backend where endpoints communicate via MPI primitives, designed for 
    high-performance computing (HPC) environments and compute clusters.
...
(** [emit_toplevel_mpi oc locations stmt_blocks] generates a complete
      ...
      - [locations]: list of endpoint names in the choreography (e.g., ["Alice"; "Bob"; "Carol"])
      - [stmt_blocks]: network IR statement blocks for each endpoint (after projection)
      
      Generates a complete runnable MPI program that:
      1. Initializes MPI and gets process rank
      2. Maps endpoint names to MPI ranks (Alice=0, Bob=1, Carol=2, ...)
      3. Branches on rank to execute the appropriate endpoint's code
      4. Uses MPI_Send/MPI_Recv for communication
      5. Finalizes MPI before exit
