  ...
  let rec find_type_sig : 'a Net.typ -> label = function
    ...
    | TLoc (_, local_type, _) ->
      let rec find_local_type_sig : 'a Local.typ -> label = function
        ...
        (* pattern matched at local Tforeign extracts string id and wraps it in parentheses,*)
      in
    ...
    | TForeign (Local.TypId (id, _), _) -> "(" ^ id ^ ")"
  in
  (* pattern match at net Tforeign extracts string id and wraps it in parentheses,*)
