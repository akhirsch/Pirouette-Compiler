(** Code generation for Domain-based communication backend.
    ...
    {b Use case:} Testing choreographies locally or running on multi-core machines
    where all endpoints can share memory space.
...
(** [emit_toplevel_domain oc locations stmt_blocks] generates a complete
      OCaml program for the Domain backend and writes it to output channel [oc].
      
      Parameters:
      - [oc]: output channel to write generated OCaml code to (e.g., file)
      - [locations]: list of endpoint names in the choreography (e.g., ["Alice"; "Bob"; "Jackie"])
      ...
      1. Creates channels connecting all endpoints
      2. Spawns a domain for each endpoint
      3. Each domain executes its projected code
      4. Main thread waits for all domains to complete 
