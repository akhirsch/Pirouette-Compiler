...
let emit_toplevel_http out_chan (loc_ids : string list)
    (net_stmtblock_l : 'a Net.stmt_block list) (config_file_path : string) =
  let emit_domain_stri (loc_id : string) (net_stmts : 'a Net.stmt_block) =
    ...
    let rec emit_net_toplevel = function
      ...
      | stmt :: stmts -> (
          ...
          | binding ->
              Ast_builder.Default.pexp_let ~loc Recursive [ binding ]
                (emit_net_toplevel stmts))
    in
    [%stri
      let () =
  ...
  Pprintast.structure
    ...
    ((warning_attr :: emit_toplevel_init loc_ids config_file_path)
    @ process_bindings)
