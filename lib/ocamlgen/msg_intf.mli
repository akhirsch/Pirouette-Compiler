(** Message interface for pluggable communication backends.
    ...
    Different implementations of this interface allow Pirouette to generate
    code for different communication mechanisms (HTTP, TCP, Unix pipes, MPI, etc.)
    without changing the core compiler.
    
    {2 Usage}
    
    The message interface separates {b what} to communicate (handled by [emit_core])
    from {b how} to communicate (handled by implementations of this interface).
...
module type M = sig
  ...
  (** [emit_net_send] generates OCaml code for sending
      ...
      - [data_expr]: Ppxlib expression containing the marshaled data
        (already wrapped with [Marshal.to_string] by [emit_core])
  ...
  (** [emit_net_recv] generates OCaml code for receiving marshaled
      ...
      Returns a [Ppxlib.expression] that represents backend-specific receive code.
      The returned expression should evaluate to a string containing the marshaled
      data, which [emit_core] will then unmarshal.
