...
let rec ast_local_pattern_alpha_rename
  : 'a Ast_core.Local.M.pattern -> 'a Ast_core.Local.M.pattern
  = function
...
let rec ast_local_type_alpha_rename : 'a Ast_core.Local.M.typ -> 'a Ast_core.Local.M.typ =
  function
  ...
  | TProd (typ1, typ2, metadata) ->
    TProd (ast_local_type_alpha_rename typ1, ast_local_type_alpha_rename typ2, metadata)
  | TSum (typ1, typ2, metadata) ->
    TSum (ast_local_type_alpha_rename typ1, ast_local_type_alpha_rename typ2, metadata)
;;
...
and ast_local_expr_alpha_rename : 'a Ast_core.Local.M.expr -> 'a Ast_core.Local.M.expr =
  function
  ...
  | Pair (expr1, expr2, metadata) ->
    Pair (ast_local_expr_alpha_rename expr1, ast_local_expr_alpha_rename expr2, metadata)
  ...
  | Match (expr, patterns, metadata) ->
    Match (ast_local_expr_alpha_rename expr, alpha_rename_pattern_match patterns, metadata)
;;

let rec ast_choreo_type_alpha_rename
  : 'a Ast_core.Choreo.M.typ -> 'a Ast_core.Choreo.M.typ
  = function
  | TUnit metadata -> TUnit metadata
  | TLoc (loc_id, local_typ, metadata) ->
    TLoc (ast_local_loc_id loc_id, ast_local_type_alpha_rename local_typ, metadata)
  | TVar (Typ_Id (type_name, type_metadata), metadata) ->
    TVar (Typ_Id (type_name ^ suffix, type_metadata), metadata)
  | TMap (typ1, typ2, metadata) ->
    TMap (ast_choreo_type_alpha_rename typ1, ast_choreo_type_alpha_rename typ2, metadata)
  | TProd (typ1, typ2, metadata) ->
    TProd (ast_choreo_type_alpha_rename typ1, ast_choreo_type_alpha_rename typ2, metadata)
  | TSum (typ1, typ2, metadata) ->
    TSum (ast_choreo_type_alpha_rename typ1, ast_choreo_type_alpha_rename typ2, metadata)
;;

let rec ast_choreo_pattern_alpha_rename
  : 'a Ast_core.Choreo.M.pattern -> 'a Ast_core.Choreo.M.pattern
  = function
  | Default metadata -> Default metadata
  ...
  | LocPat (loc_id, local_pattern, metadata) ->
    LocPat
      (ast_local_loc_id loc_id, ast_local_pattern_alpha_rename local_pattern, metadata)
...
let rec alpha_rename_pattern_match
  ...
  | [] -> []
  | (pattern, expr) :: d ->
    (ast_choreo_pattern_alpha_rename pattern, ast_choreo_expr_alpha_rename expr)
    :: alpha_rename_pattern_match d

and ast_choreo_expr_alpha_rename : 'a Ast_core.Choreo.M.expr -> 'a Ast_core.Choreo.M.expr =
  function
  | Unit metadata -> Unit metadata
  | Var (VarId (name, var_metadata), metadata) ->
    Var (VarId (name ^ suffix, var_metadata), metadata)
  | LocExpr (loc_id, local_expr, metadata) ->
    LocExpr (ast_local_loc_id loc_id, ast_local_expr_alpha_rename local_expr, metadata)
  | Send (loc_id, expr, loc_id2, metadata) ->
    Send
      ...
      , metadata )
  | Sync (loc_id, LabelId (sync_label_name, sync_metadata), loc_id2, expr, metadata) ->
    Sync
      ( ast_local_loc_id loc_id
  ...
  | Let (stmt_block, expr, metadata) ->
    Let (ast_list_alpha_rename stmt_block, ast_choreo_expr_alpha_rename expr, metadata)
  ...
  | FunApp (expr1, expr2, metadata) ->
    FunApp
      (ast_choreo_expr_alpha_rename expr1, ast_choreo_expr_alpha_rename expr2, metadata)
  | Pair (expr1, expr2, metadata) ->
    Pair (ast_choreo_expr_alpha_rename expr1, ast_choreo_expr_alpha_rename expr2, metadata)
  ...
  | Match (expr, patterns, metadata) ->
    Match
      (ast_choreo_expr_alpha_rename expr, alpha_rename_pattern_match patterns, metadata)
