(** Lexer for the Pirouette choreographic programming language.
    
    This lexer tokenizes Pirouette source code into tokens for the parser.
    It handles:
    - Keywords (fun, let, if, send, select, etc.)
    - Operators (+, -, *, /, &&, ||, etc.)
    - Literals (integers, strings, booleans)
    - Identifiers and type names
    - Comments (single-line [--] and multi-line [{- ... -}])
    
    The lexer is generated by ocamllex from this specification file. *)

{
  open Lexing
  open Parser

  (** Exception raised when invalid syntax is encountered during lexing. *)
  exception SyntaxError of string

  (** {2 {b next_line }}
  
  [next_line] advances the lexer to the next line in the input buffer.
