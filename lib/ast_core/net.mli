  ...
  type 'a expr =
    ...
    | Ret of 'a Local.M.expr * 'a
        (** Return a local expression: wraps a computation in a network context.
            ...
            {b Note:} This constructor does not have Pirouette syntax - it is
            inserted automatically by the compiler during endpoint projection to
            distinguish pure local computation from network operations.

            Used to embed local expressions (like arithmetic, variable
            references, function calls) within the network IR, marking them as
            "compute this locally, then use the result in the network context."
            Think of it as a bridge between local and network worlds.
            ...
            {[
              ...
              let ret_val =
                Ret
                  ( Local.M.BinOp
                      ( (*BinOp there are two operands*)
                        Local.M.Val (Local.M.Int (5, ()), ()),
                        (* Local Int 5*)
                        Local.M.Plus (),
                        (* Local operator *)
                        Local.M.Val (Local.M.Int (3, ()), ()),
                        (* Local Int 3 *)
                        () ),
                    (* () is the metadata*)
                    () )
    ...
    | If of 'a expr * 'a expr * 'a expr * 'a
        (** Conditional expression
            ...
            [If(cond, then_expr, else_expr, metadata)]
    ...
    | Fst of 'a expr * 'a
        (** First projection from pair
            ...
            {b OCaml:}
            {[
              let first = Snd (Var (Local.M.VarId ("z", ()), ()), ()) in
              first
            ]} *)
    | Snd of 'a expr * 'a
        (** Second projection from pair
            ...
            {b OCaml:}
            {[
              let second = Snd (Var (Local.M.VarId ("p", ()), ()), ()) in
              second
            ]} *)
    | Left of 'a expr * 'a
        (** Left injection into sum type
            ...
            {b OCaml:}
            {[
              let left_y = Right (Var (Local.M.VarId ("x", ()), ()), ()) in
              left_y
            ]} *)
    | Right of 'a expr * 'a
        (** Right injection into sum type
            ...
            {b OCaml:}
            {[
              let right_x = Right (Var (Local.M.VarId ("x", ()), ()), ()) in
              right_x
            ]} *)
  ...
  and 'a stmt =
    | Decl of 'a Local.M.pattern * 'a typ * 'a
        (** Type declaration
            ...
            {[
              let x_int_decl =
                Decl (Local.M.Var (Local.M.VarId ("x", ()), ()), TInt (), ())
              in
              x_int_decl
            ]} *)
    | Assign of 'a Local.M.pattern list * 'a expr * 'a
        (** Assignment
            ...
            {[
              let x_assign =
                Assign
                  ([ Local.M.Var (Local.M.VarId ("x", ()), ()) ], Unit (), ())
              in
              x_assign
            ]} *)
      ...
      {[
        let stmt_block =
          [
            ...
            Decl (Local.M.Var (Local.M.VarId ("y", ()), ()), TInt (), ());
