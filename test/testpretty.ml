...
let suite =
  [
    ...
    "two locations type"
    >:: test_eq_string "location {A, B}" (prettify_typ (locty [ "A"; "B" ]));
    (* Pattern Tests *)
    "wildcard pattern" >:: test_eq_string "_" (prettify_pattern wildcardpat);
    "variable pattern" >:: test_eq_string "x" (prettify_pattern (varpat "x"));
    "unit literal pattern" >:: test_eq_string "()" (prettify_pattern unitlitpat);
    "int literal pattern"
    ...
    >:: test_eq_string "\"goodbye\"" (prettify_pattern (stringlitpat "goodbye"));
    ...
    "three and a half" >:: test_eq_string "3.5" (prettify_expr three_and_a_half);
    "char c" >:: test_eq_string "'c'" (prettify_expr char_c);
    "string hello" >:: test_eq_string "\"hello\"" (prettify_expr string_hello);
    "three_plus_four" >:: test_eq_string "3 + 4" (prettify_expr three_plus_four);
    "true literal" >:: test_eq_string "true" (prettify_expr truelit);
    "false literal" >:: test_eq_string "false" (prettify_expr falselit);
    "Negation" >:: test_eq_string "-3" (prettify_expr (unop neg (intlit 3)));
    "Not" >:: test_eq_string "!true" (prettify_expr (unop not truelit));
    "type constraint"
    >:: test_eq_string "3 : int" (prettify_expr (typeconstr (intlit 3) intty));
    ...
    "recv" >:: test_eq_string "recv int from A" (prettify_expr (recv intty "A"));
    "choosefor"
    ...
    >:: test_eq_string "allow A choice \n| [L] => 3\n| [R] => 4\nend"
          (prettify_expr
             (allowchoice "A" [ (mklab "L", intlit 3); (mklab "R", intlit 4) ]));
    "ami" >:: test_eq_string "AmI A" (prettify_expr (ami (loclit "A")));
    (* Declaration Tests *)
    "emulated location"
    >:: test_eq_string "emulated location A" (prettify_decl (emlocdecl "A"));
    "type decl"
    ...
    >:: test_eq_string "foo () x true := 3 + x"
          (prettify_decl
             (defndecl "foo"
                ...
                (binop plus (intlit 3) (var "x"))));
    "import" >:: test_eq_string "import Foo" (prettify_decl (importdecl "Foo"));
    "empty variant"
    >:: test_eq_string "data false :=" (prettify_decl (variantdecl "false" []));
    "one-constr variant"
    ...
    >:: test_eq_string "data either_int_float :=\n| left : int\n| right : float"
          (prettify_decl
    ...
    >:: test_eq_string "data foo :=\n| bar : int -> float -> foo"
          (prettify_decl
             (variantdecl "foo" [ ("bar", [ intty; floatty ], varty "foo") ]));
    (* Program Tests *)
