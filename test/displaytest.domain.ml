...
let (chan_D_PIRSTDLIBLOC : string Domainslib.Chan.t) =
  Domainslib.Chan.make_bounded 0
...
let (chan_PIRSTDLIBLOC_C : string Domainslib.Chan.t) =
  Domainslib.Chan.make_bounded 0
let (chan_PIRSTDLIBLOC_D : string Domainslib.Chan.t) =
  Domainslib.Chan.make_bounded 0
;;let domain_A =
    Domain.spawn
      (fun _ ->
         ...
         let rec filenm_check_suffix_PIROUETTE_ID arg =
           Filename.check_suffix arg in
         let rec filenm_chop_suffix_PIROUETTE_ID arg =
           Filename.chop_suffix arg in
         ...
         let rec sys_is_regular_file_PIROUETTE_ID arg =
           Sys.is_regular_file arg in
         ...
         let rec sys_get_os_type_PIROUETTE_ID arg =
           (fun () -> (Sys.os_type)) arg in
         ...
         let rec err_get_backtrace_PIROUETTE_ID arg =
           Printexc.get_backtrace arg in
         ...
         let rec main_PIROUETTE_ID =
           let rec x_PIROUETTE_ID = print_sending_PIROUETTE_ID "hello from A" in
           () in
         ())
  and domain_B =
    Domain.spawn
      (fun _ ->
         ...
         let rec exit_hook_PIROUETTE_ID arg = at_exit arg in
         let rec main_PIROUETTE_ID =
           let rec y_PIROUETTE_ID = print_waiting_PIROUETTE_ID "waiting on B" in
           () in
         ())
  and domain_C =
    Domain.spawn
      (fun _ ->
         ...
         let rec exit_hook_PIROUETTE_ID arg = at_exit arg in
         let rec main_PIROUETTE_ID =
           let rec z_PIROUETTE_ID = print_idle_PIROUETTE_ID "idle" in () in
         ())
  and domain_D =
    Domain.spawn
      (fun _ ->
         ...
         let rec exit_hook_PIROUETTE_ID arg = at_exit arg in
         let rec main_PIROUETTE_ID =
           let rec p_PIROUETTE_ID = print_done_PIROUETTE_ID "D is done" in () in
         ())
  and domain_PIRSTDLIBLOC =
    Domain.spawn
      (fun _ ->
         ...
         let rec exit_PIROUETTE_ID arg = exit arg in
         let rec exit_hook_PIROUETTE_ID arg = at_exit arg in
         let rec main_PIROUETTE_ID = () in ()) in
