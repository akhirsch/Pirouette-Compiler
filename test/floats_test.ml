...
let correct_let_float_e =
  Local.Let (VarId ("foo", m), TInt m, Val (Float (1.0, m), m), float_var, m)
;;
...
let incorrect_binop_float_e =
  Local.BinOp (correct_binop_float_e, Plus m, correct_binop_int_e, m)
;;
...
let float_pattn_match =
  Local.Match
    ( Var (VarId ("foo", m), m)
    , [ left_int_p, correct_binop_float_e; right_def_p, correct_unop_float_e ]
    , m )
;;
