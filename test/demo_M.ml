...
let () =
  ...
  Send_receive.init_http_server "M" ();
  (let process_M =
     ...
     let rec main_PIROUETTE_ID =
       let rec idle_PIROUETTE_ID = print_idle_PIROUETTE_ID "[M] idle" in
       let rec nap_PIROUETTE_ID = sleep_PIROUETTE_ID 1 in
       let rec waiting_PIROUETTE_ID =
         print_waiting_PIROUETTE_ID "M waiting on G..." in
       let rec inbox_PIROUETTE_ID =
         Marshal.from_string (Send_receive.receive_message ~location:"M") 0 in
       let rec nap_PIROUETTE_ID = sleep_PIROUETTE_ID 1 in
       let rec received_PIROUETTE_ID =
         print_done_PIROUETTE_ID "M received from G!" in
       let rec nap_PIROUETTE_ID = sleep_PIROUETTE_ID 1 in
       let rec sending_PIROUETTE_ID =
         print_sending_PIROUETTE_ID "M sending to D..." in
       let rec _unit_27 =
         let val_26 = inbox_PIROUETTE_ID in
         match Lwt_main.run
                 (Send_receive.send_message ~location:"D"
                    ~data:(Marshal.to_string val_26 []))
         with
         | Ok () -> ()
         | Error msg -> failwith ("Send error: " ^ msg) in
       let rec nap_PIROUETTE_ID = sleep_PIROUETTE_ID 1 in
       let rec done_PIROUETTE_ID =
         print_done_PIROUETTE_ID "M send to D sucessful!" in
       let rec nap_PIROUETTE_ID = () in () in
     () in
   ignore process_M)