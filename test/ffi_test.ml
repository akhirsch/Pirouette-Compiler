...
let test_parse_external_name_invalid () =
  assert_raises
    ...
    (fun () -> Ast_utils.parse_external_name ":")
;;
...
let test_parse_external_name_multiple_at () =
  assert_raises
    (Failure "Impossible Failure")
    (fun () -> Ast_utils.parse_external_name "Pet:feed@path1@path2")
;;

(* verifies that a single ForeignDecl is collected *)
let test_collect_ffi_info_single () =
  let var_id = Local.M.VarId ("f", 1) in (* f is the name of the foreign function *)
...
let test_ast_info_map_foreigndecl () =
  ...
  let result = Ast_utils.ast_list_info_map (fun _ -> 2) [stmt] in (* mapping over the stmt replacing meta 1 with 2 *)
  assert_equal [Choreo.M.ForeignDecl (Local.M.VarId ("f", 2), Choreo.M.TUnit 2, "Pet:feed", 2)] result
...
let test_ast_info_map_foreigntypedecl () =
  ...
  assert_equal [Choreo.M.ForeignTypeDecl (Local.M.TypId ("Int32", 2), 2)] result
;;
...
let test_ast_local_type_info_map_tforeign () =
  let var_id = Local.M.VarId ("f", 1) in
  let loc_id = Local.M.LocId ("Alice", 1) in (* creaes the location alice withg meta 1 *)
  let typ_id = Local.M.TypId ("Int32", 1) in (* create the typeid Int32 with meta 1 *)
  let local_typ = Local.M.TForeign (typ_id, 1) in (* wrapping Int32 as TForeign with meta 1 the node to be tested! *)
  let typ = Choreo.M.TLoc (loc_id, local_typ, 1) in (* creating the type signature Alice.Int32 *)
  let stmt = Choreo.M.ForeignDecl (var_id, typ, "Pet:feed", 1) in
  let result = Ast_utils.ast_list_info_map (fun _ -> 2) [stmt] in
  ...
  let expected_local_typ = Local.M.TForeign (Local.M.TypId ("Int32", 2), 2) in (* the expected result after mapping
  TForeign with metadata updated to 2 and type name Int32 preserved*)
  let expected_typ = Choreo.M.TLoc (Local.M.LocId ("Alice", 2), expected_local_typ, 2) in
  assert_equal [Choreo.M.ForeignDecl (Local.M.VarId ("f", 2), expected_typ, "Pet:feed", 2)] result
...
let test_ast_choreo_type_info_map_tforeign () =
  let var_id = Local.M.VarId ("f", 1) in
  let typ_id = Choreo.M.Typ_Id ("Int32", 1) in
  let typ = Choreo.M.TForeign (typ_id, 1) in (* constructing what we are testing *)
  let stmt = Choreo.M.ForeignDecl (var_id, typ, "Pet:feed", 1) in (* Put it inside a Choreo.ForeignDecl as the type signature
  because ForeignDecl is a statement that carries a choreo type *)
  let result = Ast_utils.ast_list_info_map (fun _ -> 2) [stmt] in
  assert_equal [Choreo.M.ForeignDecl (Local.M.VarId ("f", 2), Choreo.M.TForeign (Choreo.M.Typ_Id ("Int32", 2), 2), "Pet:feed", 2)] result
...
let net_binding_test ~node_id stmt pattern_test expr_test =
  let binding = emit_net_binding ~self_id:node_id (module Ocamlgen.Toplevel_http.Msg_http_intf) stmt in
...
let net_binding_foreign_decl_net_tforeign _ =
  ...
  let pattern_test pat =
    ...
    | _ -> false
  in
  assert_equal true (net_binding_test ~node_id:"testing" stmt pattern_test "Pet.feed")
;;
...
let emit_core_ffi_suite = 
  "Emit core Tests"
  >::: [ "find_local_type_sig at net level TForeign" >:: (fun _ -> net_binding_foreign_decl_local_tforeign ())
        ; "find_local_type_sig nested TForeign" >:: ( fun _ -> net_binding_foreign_decl_net_tforeign ())
...
let ast_locs_ffi_suite =
  ...
  >::: [ "ForeignTypeDecl no locs" >:: (fun _ -> test_foreigntypedecl_no_locs ())
       ...
       ; "TForeign extract_type no locs" >:: (fun _ -> test_tforeign_extract_type_no_locs ())
       ]
;;

let ast_utils_ffi_suite =
  "Ast Utils FFI Tests"
  >::: [ "parse_external_name simple" >:: (fun _ -> test_parse_external_name_simple ())
       ; "parse_external_name with package" >:: (fun _ -> test_parse_external_name_with_package ())
       ; "parse_external_name with search path" >:: (fun _ -> test_parse_external_name_with_search_path ())
       ; "parse_external_name nested module" >:: (fun _ -> test_parse_external_name_nested_module ())
       ; "collect_ffi_info empty" >:: (fun _ -> test_collect_ffi_info_empty ())
       ; "collect_ffi_info single" >:: (fun _ -> test_collect_ffi_info_single ())
       ; "collect_ffi_info ignores other stmts" >:: (fun _ -> test_collect_ffi_info_ignores_other_stmts ())
       ; "collect_ffi_info deduplicates" >:: (fun _ -> test_collect_ffi_info_deduplicates ())
       ; "ast_list_info_map local TForeign" >:: (fun _ -> test_ast_local_type_info_map_tforeign ())
       ; "ast_list_info_map choreo TForeign" >:: (fun _ -> test_ast_choreo_type_info_map_tforeign ())
       ; "ast_list_info_map ForeignDecl" >:: (fun _ -> test_ast_info_map_foreigndecl ())
       ; "ast_list_info_map ForeignTypeDecl" >:: (fun _ -> test_ast_info_map_foreigntypedecl ())
       ; "parse_external_name invalid format" >:: (fun _ -> test_parse_external_name_invalid ())
       ; "parse_external_name multiple @ symbols" >:: (fun _ -> test_parse_external_name_multiple_at () )
...
let () =
  ...
  run_test_tt_main ast_locs_ffi_suite;
  run_test_tt_main ast_utils_ffi_suite;
  run_test_tt_main emit_core_ffi_suite
;;