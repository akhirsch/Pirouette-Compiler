...
let test_ast_info_map_foreigndecl () =
  ...
  assert_equal
    [ Choreo.M.ForeignDecl (Local.M.VarId ("f", 2), Choreo.M.TUnit 2, "Pet:feed", 2) ]
    result
;;
...
let net_binding_test ~node_id stmt pattern_test expr_test =
  let binding =
    emit_net_binding ~self_id:node_id (module Ocamlgen.Toplevel_http.Msg_http_intf) stmt
  in
...
let emit_core_ffi_suite =
  "Emit core Tests"
  >::: [ ("find_local_type_sig at net level TForeign"
          >:: fun _ -> net_binding_foreign_decl_local_tforeign ())
       ; ("find_local_type_sig nested TForeign"
          >:: fun _ -> net_binding_foreign_decl_net_tforeign ())
       ]
;;
...
let ast_locs_ffi_suite =
  ...
  >::: [ ("ForeignTypeDecl no locs" >:: fun _ -> test_foreigntypedecl_no_locs ())
       ...
       ; ("TForeign extract_type no locs"
          >:: fun _ -> test_tforeign_extract_type_no_locs ())
       ]
;;
...
let ast_utils_ffi_suite =
  ...
  >::: [ ("parse_external_name simple" >:: fun _ -> test_parse_external_name_simple ())
       ; ("parse_external_name with package"
          >:: fun _ -> test_parse_external_name_with_package ())
       ; ("parse_external_name with search path"
          >:: fun _ -> test_parse_external_name_with_search_path ())
       ; ("parse_external_name nested module"
          >:: fun _ -> test_parse_external_name_nested_module ())
       ; ("collect_ffi_info empty" >:: fun _ -> test_collect_ffi_info_empty ())
       ; ("collect_ffi_info single" >:: fun _ -> test_collect_ffi_info_single ())
       ; ("collect_ffi_info ignores other stmts"
          >:: fun _ -> test_collect_ffi_info_ignores_other_stmts ())
       ; ("collect_ffi_info deduplicates"
          >:: fun _ -> test_collect_ffi_info_deduplicates ())
       ; ("ast_list_info_map local TForeign"
          >:: fun _ -> test_ast_local_type_info_map_tforeign ())
       ; ("ast_list_info_map choreo TForeign"
          >:: fun _ -> test_ast_choreo_type_info_map_tforeign ())
       ; ("ast_list_info_map ForeignDecl" >:: fun _ -> test_ast_info_map_foreigndecl ())
       ; ("ast_list_info_map ForeignTypeDecl"
          >:: fun _ -> test_ast_info_map_foreigntypedecl ())
       ; ("parse_external_name invalid format"
          >:: fun _ -> test_parse_external_name_invalid ())
       ; ("parse_external_name multiple @ symbols"
          >:: fun _ -> test_parse_external_name_multiple_at ())
       ]
;;
