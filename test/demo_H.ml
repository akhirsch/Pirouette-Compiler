...
let () =
  ...
  Send_receive.init_http_server "H" ();
  (let process_H =
     ...
     let rec main_PIROUETTE_ID =
       let rec idle_PIROUETTE_ID = print_idle_PIROUETTE_ID "[H] idle" in
       let rec nap_PIROUETTE_ID = sleep_PIROUETTE_ID 1 in
       let rec waiting_PIROUETTE_ID =
         print_waiting_PIROUETTE_ID "H waiting on L..." in
       let rec inbox_PIROUETTE_ID =
         Marshal.from_string (Send_receive.receive_message ~location:"H") 0 in
       let rec nap_PIROUETTE_ID = sleep_PIROUETTE_ID 1 in
       let rec received_PIROUETTE_ID =
         print_done_PIROUETTE_ID "H received from L!" in
       let rec nap_PIROUETTE_ID = sleep_PIROUETTE_ID 1 in
       let rec sending_PIROUETTE_ID =
         print_sending_PIROUETTE_ID "H sending to A..." in
       let rec _unit_17 =
         let val_16 = inbox_PIROUETTE_ID in
         match Lwt_main.run
                 (Send_receive.send_message ~location:"A"
                    ~data:(Marshal.to_string val_16 []))
         with
         | Ok () -> ()
         | Error msg -> failwith ("Send error: " ^ msg) in
       let rec nap_PIROUETTE_ID = sleep_PIROUETTE_ID 1 in
       let rec done_PIROUETTE_ID =
         print_done_PIROUETTE_ID "H send to A sucessful!" in
       let rec nap_PIROUETTE_ID = sleep_PIROUETTE_ID 3 in () in
     () in
   ignore process_H)