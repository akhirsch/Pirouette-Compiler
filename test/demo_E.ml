...
let () =
  ...
  Send_receive.init_http_server "E" ();
  (let process_E =
     ...
     let rec main_PIROUETTE_ID =
       ...
       let rec inbox_PIROUETTE_ID =
         Marshal.from_string (Send_receive.receive_message ~location:"E") 0 in
       let rec nap_PIROUETTE_ID = sleep_PIROUETTE_ID 1 in
       let rec received_PIROUETTE_ID =
         print_done_PIROUETTE_ID "E received from N!" in
       let rec nap_PIROUETTE_ID = sleep_PIROUETTE_ID 1 in
       let rec sending_PIROUETTE_ID =
         print_sending_PIROUETTE_ID "E sending to L..." in
       let rec _unit_10 =
         let val_9 = inbox_PIROUETTE_ID in
         match Lwt_main.run
                 (Send_receive.send_message ~location:"L"
       ...
       let rec done_PIROUETTE_ID =
         print_done_PIROUETTE_ID "E send to L sucessful!" in
       () in
     () in
   ignore process_E)