...
let () =
  ...
  Send_receive.init_http_server "G" ();
  (let process_G =
     ...
     let rec main_PIROUETTE_ID =
       let rec idle_PIROUETTE_ID = print_idle_PIROUETTE_ID "[G] idle" in
       let rec waiting_PIROUETTE_ID =
         print_waiting_PIROUETTE_ID "[G] waiting on A..." in
       let rec inbox_PIROUETTE_ID =
         Marshal.from_string (Send_receive.receive_message ~location:"G") 0 in
       let rec sending_PIROUETTE_ID =
         print_sending_PIROUETTE_ID "[G] sending to M..." in
       let rec _unit_4 =
         let val_3 = inbox_PIROUETTE_ID in
         match Lwt_main.run
                 (Send_receive.send_message ~location:"M"
                    ~data:(Marshal.to_string val_3 []))
         with
         | Ok () -> ()
         | Error msg -> failwith ("Send error: " ^ msg) in
       let rec done_PIROUETTE_ID = print_done_PIROUETTE_ID "[G] done" in
       let rec nap_PIROUETTE_ID = sleep_PIROUETTE_ID 1 in () in
     () in
   ignore process_G)