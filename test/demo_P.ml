...
let () =
  ...
  Send_receive.init_http_server "P" ();
  (let process_P =
     ...
     let rec main_PIROUETTE_ID =
       let rec idle_PIROUETTE_ID = print_idle_PIROUETTE_ID "[P] idle" in
       let rec nap_PIROUETTE_ID = sleep_PIROUETTE_ID 1 in
       let rec waiting_PIROUETTE_ID =
         print_waiting_PIROUETTE_ID "P waiting on C..." in
       let rec inbox_PIROUETTE_ID =
         Marshal.from_string (Send_receive.receive_message ~location:"P") 0 in
       let rec nap_PIROUETTE_ID = sleep_PIROUETTE_ID 1 in
       let rec received_PIROUETTE_ID =
         print_done_PIROUETTE_ID "P received from C!" in
       let rec nap_PIROUETTE_ID = sleep_PIROUETTE_ID 1 in
       let rec sending_PIROUETTE_ID =
         print_sending_PIROUETTE_ID "P sending to I..." in
       let rec _unit_34 =
         let val_33 = inbox_PIROUETTE_ID in
         match Lwt_main.run
                 (Send_receive.send_message ~location:"I"
                    ~data:(Marshal.to_string val_33 []))
         with
         | Ok () -> ()
         | Error msg -> failwith ("Send error: " ^ msg) in
       let rec nap_PIROUETTE_ID = sleep_PIROUETTE_ID 1 in
       let rec done_PIROUETTE_ID =
         print_done_PIROUETTE_ID "P send to I sucessful!" in
       () in
     () in
   ignore process_P)