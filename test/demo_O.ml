...
let () =
  ...
  Send_receive.init_http_server "O" ();
  (let process_O =
     ...
     let rec main_PIROUETTE_ID =
       let rec idle_PIROUETTE_ID = print_idle_PIROUETTE_ID "[O] idle" in
       let rec nap_PIROUETTE_ID = sleep_PIROUETTE_ID 1 in
       let rec waiting_PIROUETTE_ID =
         print_waiting_PIROUETTE_ID "O waiting on B..." in
       let rec inbox_PIROUETTE_ID =
         Marshal.from_string (Send_receive.receive_message ~location:"O") 0 in
       let rec nap_PIROUETTE_ID = sleep_PIROUETTE_ID 1 in
       let rec received_PIROUETTE_ID =
         print_done_PIROUETTE_ID "O received from B!" in
       let rec nap_PIROUETTE_ID = sleep_PIROUETTE_ID 1 in
       ...
       let rec done_PIROUETTE_ID =
         print_done_PIROUETTE_ID "O send to F sucessful!" in
       () in
     () in
   ignore process_O)