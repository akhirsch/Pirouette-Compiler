...
[@@@ocaml.warning "-39"]
let () =
  let config_file_path : string = "test/shortdemo.yaml" in
...
let () =
  ...
  Send_receive.init_http_server "A" ();
  (let process_A =
     ...
     let rec main_PIROUETTE_ID =
       let rec idle_PIROUETTE_ID = print_idle_PIROUETTE_ID "[A] idle" in
       let rec start_PIROUETTE_ID = sleep_PIROUETTE_ID 1 in
       let rec inbox_PIROUETTE_ID = "hot potato" in
       let rec sending_PIROUETTE_ID =
         print_sending_PIROUETTE_ID "[A] sending to G..." in
       let rec _unit_2 =
         let val_1 = inbox_PIROUETTE_ID in
         match Lwt_main.run
                 (Send_receive.send_message ~location:"G"
                    ~data:(Marshal.to_string val_1 []))
         with
         | Ok () -> ()
         | Error msg -> failwith ("Send error: " ^ msg) in
       let rec done_PIROUETTE_ID = print_done_PIROUETTE_ID "[A] done" in
       let rec nap_PIROUETTE_ID = sleep_PIROUETTE_ID 1 in
       let rec waiting_PIROUETTE_ID =
         print_waiting_PIROUETTE_ID "[A] waiting on M..." in
       let rec final_PIROUETTE_ID =
         Marshal.from_string (Send_receive.receive_message ~location:"A") 0 in
       let rec done2_PIROUETTE_ID =
         print_done_PIROUETTE_ID "[A] received! done" in
       () in
     () in
   ignore process_A)