...
let test_expression_right_LOC (old_meta : int) (new_meta : int) =
  ...
  let (new_right : int Local.M.expr) =
    LocalAst.set_info_expr new_meta var_right
  in
...
let test_expression_left_LOC (old_meta : int) (new_meta : int) =
  ...
  let (new_left : int Local.M.expr) =
    LocalAst.set_info_expr new_meta var_left
  in
...
let test_expression_pair_LOC (old_meta : int) (new_meta : int) =
  ...
  let (pair_expr : int Local.M.expr) =
    Local.M.Pair (expr_pair1, expr_pair2, old_meta)
  in
  let (new_pair : int Local.M.expr) =
    LocalAst.set_info_expr new_meta pair_expr
  in
  assert_equal new_meta (LocalAst.get_info_expr new_pair)

let test_expression_let_LOC (old_meta : int) (new_meta : int) (input_int : int)
    (input_int2 : int) =
...
let test_expression_binop_LOC (old_meta : int) (new_meta : int) =
  ...
  let (bi_op : int Local.M.expr) =
    Local.M.BinOp (var_expr, plus, var_expr, 1)
  in
...
let test_expression_var_LOC (old_meta : int) (new_meta : int) =
  ...
  let (new_expr : int Local.M.expr) =
    LocalAst.set_info_expr new_meta var_expr
  in
...
let test_expression_val_LOC (old_meta : int) (new_meta : int) =
  ...
  let (new_expr : int Local.M.expr) =
    LocalAst.set_info_expr new_meta int_expr
  in
  assert_equal new_meta (LocalAst.get_info_expr new_expr)
...
let test_expression_unit_LOC (new_meta : int) =
  ...
  let (new_expr : int Local.M.expr) =
    LocalAst.set_info_expr new_meta val_unit
  in
  assert_equal new_meta (LocalAst.get_info_expr new_expr)
...
let test_pattern_pair_LOC (old_meta : int) (new_meta : int) =
  ...
  let (pat_pair : int Local.M.pattern) =
    Local.M.Pair (pat_pair1, pat_pair2, old_meta)
  in
...
let test_pattern_right_LOC (old_meta : int) (new_meta : int) =
  ...
  let (val_pat_right : int Local.M.pattern) =
    Local.M.Right (var_pat1, old_meta)
  in
...
let test_pattern_left_LOC (old_meta : int) (new_meta : int) =
  let var_id = Local.M.VarId ("hi", 1) in
  let (var_pat1 : int Local.M.pattern) = Local.M.Var (var_id, old_meta) in
  let (val_pat_left : int Local.M.pattern) =
    Local.M.Left (var_pat1, old_meta)
  in
  let (new_pat : int Local.M.pattern) =
    LocalAst.set_info_pattern new_meta val_pat_left
  in
  assert_equal new_meta (LocalAst.get_info_pattern new_pat)
...
let test_pattern_variable_LOC (old_meta : int) (new_meta : int) =
  ...
  let (new_pat : int Local.M.pattern) =
    LocalAst.set_info_pattern new_meta val_pat
  in
  assert_equal new_meta (LocalAst.get_info_pattern new_pat)
...
let test_pattern_value_LOC (old_meta : int) (new_meta : int) =
  ...
  let (new_pat : int Local.M.pattern) =
    LocalAst.set_info_pattern new_meta pat
  in
  assert_equal new_meta (LocalAst.get_info_pattern new_pat)
...
let test_type_sum_LOC (old_meta : int) (new_meta : int) (input_int : int)
    (input_int2 : int) =
  let old_sum =
    Local.M.TSum (Local.M.TUnit old_meta, Local.M.TString input_int, input_int2)
  in
...
let test_type_prod_LOC (old_meta : int) (new_meta : int) (input_int : int)
    (input_int2 : int) =
  let old_prod =
...
let test_change_string_LOC (new_meta : int) (old_meta : int)
    (input_string : string) =
...
let loc_suite =
  ...
  >::: [
         ...
         >::: [
                ...
                ( "test_change_int (-1) (-2)" >:: fun _ ->
                  test_change_int_LOC (-1) (-2) );
                ( "test_change_int 1000 2000" >:: fun _ ->
                  test_change_int_LOC 1000 2000 );
              ];
         "String Tests"
         >::: [
                ( "test_change_string 1 2 \"hello\"" >:: fun _ ->
                  test_change_string_LOC 1 2 "hello" );
                ( "test_change_string 2 3 \"hellonjdvsbvjsbgjkbs\"" >:: fun _ ->
                  test_change_string_LOC 2 3 "hellonjdvsbvjsbgjkbs" );
                ( "test_change_string 10 20 \"hi\"" >:: fun _ ->
                  test_change_string_LOC 10 20 "hi" );
                ( "test_change_string 1000 2000 \"hello\"" >:: fun _ ->
                  test_change_string_LOC 1000 2000 "hello" );
                ( "test_change_string (-1) (-2) \"hello\"" >:: fun _ ->
                  test_change_string_LOC (-1) (-2) "hello" );
              ];
         "Bool Tests"
         >::: [
                ( "test_change_bool 1 2 false" >:: fun _ ->
                  test_change_bool_LOC 1 2 false );
                ( "test_change_bool 2 3 true" >:: fun _ ->
                  test_change_bool_LOC 2 3 true );
                ( "test_change_bool 10 20 false" >:: fun _ ->
                  test_change_bool_LOC 10 20 false );
                ( "test_change_bool 1000 2000 true" >:: fun _ ->
                  test_change_bool_LOC 1000 2000 true );
                ( "test_change_bool (-1) (-2) false" >:: fun _ ->
                  test_change_bool_LOC (-1) (-2) false );
              ];
         "Operators Tests"
         ...
         >::: [
                ...
                ("test_type_string 1 2" >:: fun _ -> test_type_string_LOC 1 2);
                ("test_type_bool 1 2" >:: fun _ -> test_type_bool_LOC 1 2);
                ( "test_type_prod 1 2 3 4" >:: fun _ ->
                  test_type_prod_LOC 1 2 3 4 );
         ...
         "Pattern Tests"
         >::: [
                ( "test_pattern_default 1 2" >:: fun _ ->
                  test_pattern_default_LOC 1 2 );
                ( "test_pattern_value 1 2" >:: fun _ ->
                  test_pattern_value_LOC 1 2 );
                ( "test_pattern_variable 1 2" >:: fun _ ->
                  test_pattern_variable_LOC 1 2 );
         ...
         "Expression Tests"
         >::: [
                ( "test_expression_unit 2" >:: fun _ ->
                  test_expression_unit_LOC 2 );
                ( "test_expression_val 1 2" >:: fun _ ->
                  test_expression_val_LOC 1 2 );
                ( "test_expression_unop 1 2" >:: fun _ ->
                  test_expression_unop_LOC 1 2 );
                ( "test_expression_var 1 2" >:: fun _ ->
                  test_expression_var_LOC 1 2 );
                ( "test_expression_binop 1 2" >:: fun _ ->
                  test_expression_binop_LOC 1 2 );
                ( "test_expression_let 1 2 3 4" >:: fun _ ->
                  test_expression_let_LOC 1 2 3 4 );
                ( "test_expression_fst 1 2" >:: fun _ ->
                  test_expression_fst_LOC 1 2 );
                ( "test_expression_snd 1 2" >:: fun _ ->
                  test_expression_snd_LOC 1 2 );
                ( "test_expression_left 1 2" >:: fun _ ->
                  test_expression_left_LOC 1 2 );
                ( "test_expression_right 1 2" >:: fun _ ->
                  test_expression_right_LOC 1 2 );
                ( "test_expression_match 1 2" >:: fun _ ->
...
let test_pattern_variable_CH (old_meta : int) (new_meta : int) =
  ...
  let (new_info : int Choreo.M.pattern) =
    ChoreoAst.set_info_pattern new_meta val_pat
  in
  assert_equal new_meta (ChoreoAst.get_info_pattern new_info)
...
let test_pattern_pair_CH (old_meta : int) (new_meta : int) =
  ...
  let (val_pat : int Choreo.M.pattern) =
    Choreo.M.Pair (pat_pair1, pat_pair2, old_meta)
  in
  let (new_info : int Choreo.M.pattern) =
    ChoreoAst.set_info_pattern new_meta val_pat
  in
...
let test_get_info_sync_CH (meta1 : int) (meta2 : int) (meta3 : int)
    (meta4 : int) =
  let loc_id1 = Local.M.LocId ("meta1", meta1) in
...
let test_get_info_pair_CH (meta1 : int) (meta2 : int) =
  ...
  let typ_var : int Choreo.M.expr =
    Choreo.M.Pair (pat_pair1, pat_pair2, meta1)
  in
...
let test_get_info_match_CH (meta1 : int) (meta2 : int) =
  let exp = Choreo.M.Unit 1 in
  let pat = Choreo.M.Default meta1 in
  let cases : (int Choreo.M.pattern * int Choreo.M.expr) list =
    [ (pat, exp) ]
  in
...
let test_set_info_locexpr_CH (old_meta1 : int) (old_meta2 : int)
    (new_meta : int) =
...
let test_set_info_send_CH (meta1 : int) (meta2 : int) (meta3 : int)
    (new_meta : int) =
  let loc_id1 = Local.M.LocId ("meta1", meta1) in
  let exp = Choreo.M.Unit 1 in
  let loc_id2 = Local.M.LocId ("meta2", meta2) in
...
let test_set_info_sync_CH (meta1 : int) (meta2 : int) (meta3 : int)
    (meta4 : int) (new_meta : int) =
  let loc_id1 = Local.M.LocId ("meta1", meta1) in
  let label = Local.M.LabelId ("meta2", meta2) in
  let loc_id2 = Local.M.LocId ("meta3", meta3) in
  let exp = Choreo.M.Unit 1 in
...
let test_set_info_if_CH (meta1 : int) (meta2 : int) (meta3 : int) (meta4 : int)
    (new_meta : int) =
  let m1 = Local.M.VarId ("m1", meta1) in
  let e1 = Choreo.M.Var (m1, meta1) in
  let m2 = Local.M.VarId ("m2", meta2) in
...
let test_set_info_let_CH (meta1 : int) (meta2 : int) (meta3 : int)
    (new_meta : int) =
  let pat = Choreo.M.Default meta1 in
  let typ = Choreo.M.TUnit meta1 in
  let dec1 = Choreo.M.Decl (pat, typ, meta1) in
...
let choreo_suite =
  ...
  >::: [
         "CH Get + Set Info Pattern Tests"
         >::: [
                ( "test_pattern_default_CH" >:: fun _ ->
                  test_pattern_default_CH 1 2 );
                ( "test_pattern_variable_CH" >:: fun _ ->
                  test_pattern_variable_CH 1 2 );
                ("test_pattern_pair_CH" >:: fun _ -> test_pattern_pair_CH 1 2);
                ( "test_pattern_locpat_CH" >:: fun _ ->
         ...
         >::: [
                ...
                ( "test_get_info_locexpr_CH" >:: fun _ ->
                  test_get_info_locexpr_CH 1 2 );
                ( "test_get_info_send_CH" >:: fun _ ->
                  test_get_info_send_CH 1 2 3 );
                ( "test_get_info_sync_CH" >:: fun _ ->
                  test_get_info_sync_CH 1 2 3 4 );
                ("test_get_info_if_CH" >:: fun _ -> test_get_info_if_CH 1 2 3 4);
                ("test_get_info_let_CH" >:: fun _ -> test_get_info_let_CH 1 2 3);
                ( "test_get_info_fundef_CH" >:: fun _ ->
                  test_get_info_fundef_CH 1 2 );
                ( "test_get_info_funapp_CH" >:: fun _ ->
                  test_get_info_funapp_CH 1 );
         ...
         "CH Set Info Typ Tests"
         >::: [
                ( "test_set_info_typid_CH" >:: fun _ ->
                  test_set_info_typid_CH 3 4 );
                ( "test_set_info_tunit_CH" >:: fun _ ->
         ...
         >::: [
                ...
                ( "test_set_info_locexpr_CH" >:: fun _ ->
                  test_set_info_locexpr_CH 3 4 5 );
                ( "test_set_info_send_CH" >:: fun _ ->
                  test_set_info_send_CH 3 4 5 6 );
                ( "test_set_info_sync_CH" >:: fun _ ->
                  test_set_info_sync_CH 3 4 5 6 7 );
                ( "test_set_info_if_CH" >:: fun _ ->
                  test_set_info_if_CH 3 4 5 6 7 );
                ( "test_set_info_let_CH" >:: fun _ ->
                  test_set_info_let_CH 3 4 5 6 );
                ( "test_set_info_fundef_CH" >:: fun _ ->
                  test_set_info_fundef_CH 3 4 5 );
                ( "test_set_info_funapp_CH" >:: fun _ ->
                  test_set_info_funapp_CH 3 4 );
                ( "test_set_info_pair_CH" >:: fun _ ->
                  test_set_info_pair_CH 3 4 5 );
         ...
         >::: [
                ( "test_info_stmt_decl_CH" >:: fun _ ->
                  test_info_stmt_decl_CH 1 2 3 );
                ( "test_info_stmt_assign_CH" >:: fun _ ->
                  test_info_stmt_assign_CH 1 2 );
                ( "test_info_typedecl_CH" >:: fun _ ->
                  test_info_stmt_typedecl_CH 1 2 3 );
                ( "test_info_foreigndecl_CH" >:: fun _ ->
