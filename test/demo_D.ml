...
let () =
  ...
  Send_receive.init_http_server "D" ();
  (let process_D =
     ...
     let rec main_PIROUETTE_ID =
       let rec idle_PIROUETTE_ID = print_idle_PIROUETTE_ID "[D] idle" in
       ...
       let rec sending_PIROUETTE_ID =
         print_sending_PIROUETTE_ID "D sending to J..." in
       let rec _unit_8 =
         let val_7 = inbox_PIROUETTE_ID in
         match Lwt_main.run
                 (Send_receive.send_message ~location:"J"
       ...
       let rec done_PIROUETTE_ID =
         print_done_PIROUETTE_ID "D send to J sucessful!" in
       () in
     () in
   ignore process_D)