...
let local_binop_plus _ =
  let expr =
    BinOp (Val (Int (5, ()), ()), Plus (), Val (Int (3, ()), ()), ())
  in
  let result = expr_to_string (emit_local_pexp expr) in
  assert_equal true (contains_substring result "5");
  assert_equal true (contains_substring result "+");
...
let local_binop_minus _ =
  let expr =
    BinOp (Val (Int (10, ()), ()), Minus (), Val (Int (4, ()), ()), ())
  in
  let result = expr_to_string (emit_local_pexp expr) in
...
let local_binop_times _ =
  let expr =
    BinOp (Val (Int (5, ()), ()), Times (), Val (Int (7, ()), ()), ())
  in
  let result = expr_to_string (emit_local_pexp expr) in
  assert_equal true (contains_substring result "5");
  assert_equal true (contains_substring result "*");
...
let local_binop_and _ =
  let expr =
    BinOp (Val (Bool (true, ()), ()), And (), Val (Bool (false, ()), ()), ())
  in
  let result = expr_to_string (emit_local_pexp expr) in
  assert_equal true (contains_substring result "true");
  assert_equal true (contains_substring result "&&");
...
let local_binop_or _ =
  let expr =
    BinOp (Val (Bool (true, ()), ()), Or (), Val (Bool (false, ()), ()), ())
  in
  let result = expr_to_string (emit_local_pexp expr) in
  assert_equal true (contains_substring result "true");
  assert_equal true (contains_substring result "||");
...
let local_binop_eq _ =
  let expr =
    BinOp (Val (Int (15, ()), ()), Eq (), Val (Int (15, ()), ()), ())
  in
  let result = expr_to_string (emit_local_pexp expr) in
...
let local_binop_div _ =
  let expr =
    BinOp (Val (Int (10, ()), ()), Div (), Val (Int (2, ()), ()), ())
  in
  let result = expr_to_string (emit_local_pexp expr) in
  assert_equal true (contains_substring result "10");
  assert_equal true (contains_substring result "/");
...
let local_binop_leq _ =
  let expr =
    BinOp (Val (Int (2, ()), ()), Leq (), Val (Int (12, ()), ()), ())
  in
  let result = expr_to_string (emit_local_pexp expr) in
  assert_equal true (contains_substring result "2");
  assert_equal true (contains_substring result "<=");
...
let local_binop_geq _ =
  let expr =
    BinOp (Val (Int (15, ()), ()), Geq (), Val (Int (1, ()), ()), ())
  in
  let result = expr_to_string (emit_local_pexp expr) in
  assert_equal true (contains_substring result "15");
  assert_equal true (contains_substring result ">=");
...
let local_fst _ =
  let expr =
    Fst (Pair (Val (Int (1, ()), ()), Val (Bool (true, ()), ()), ()), ())
  in
  let result = expr_to_string (emit_local_pexp expr) in
...
let local_snd _ =
  let expr =
    Snd (Pair (Val (Int (1, ()), ()), Val (Bool (true, ()), ()), ()), ())
  in
  let result = expr_to_string (emit_local_pexp expr) in
...
let local_pat_int _ =
  let expr =
    Match (Val (Int (7, ()), ()), [ (Val (Int (7, ()), ()), Unit ()) ], ())
  in
  let result = expr_to_string (emit_local_pexp expr) in
  assert_equal true (contains_substring result "match");
  assert_equal true (contains_substring result "7");
...
let net_if _ =
  let expr =
    Net.If
      (Ret (Local.Val (Local.Bool (true, ()), ()), ()), Unit (), Unit (), ())
  in
  let result = net_expr_to_string ~node_id:"testing" expr in
...
let net_send _ =
  let expr =
    Net.Send
      ( Ret (Local.Val (Local.String ("hello", ()), ()), ()),
        LocId ("Alice", ()),
        () )
  in
  let result = net_expr_to_string ~node_id:"testing" expr in
...
let net_binding_var _ =
  ...
  in
  assert_equal true
    (net_binding_test ~node_id:"testing" stmt pattern_test "123")
...
let net_binding_fun _ =
  ...
  in
  assert_equal true
    (net_binding_test ~node_id:"testing" stmt pattern_test "fun x -> y")

let net_binding_default _ =
  let stmt =
    Net.Assign
      ([ Local.Default () ], Ret (Local.Val (Local.Int (715, ()), ()), ()), ())
  in
  let pattern_test pat =
    match pat.ppat_desc with
  ...
  in
  assert_equal true
    (net_binding_test ~node_id:"testing" stmt pattern_test "715")
...
let net_binding_foreign_decl _ =
  ...
  in
  assert_equal true
    (net_binding_test ~node_id:"testing" stmt pattern_test
...
let test_package_submodule_external_function _ =
  ...
  assert_equal
    ...
    "fun arg -> Math.Deep.calculate arg" (String.trim result)
