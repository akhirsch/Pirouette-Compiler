...
let correct_binop_bool_e =
  Local.BinOp (Val (Int (1, m), m), Eq m, Val (Int (1, m), m), m)

let correct_binop_int_e =
  Local.BinOp (Val (Int (1, m), m), Plus m, Val (Int (1, m), m), m)

let correct_unop_bool_e = Local.UnOp (Not m, correct_binop_bool_e, m)

let correct_and_bool_e =
  Local.BinOp (correct_binop_bool_e, And m, correct_unop_bool_e, m)

let correct_unop_int_e = Local.UnOp (Neg m, correct_binop_int_e, m)

let correct_pair_e =
  Local.Pair (correct_binop_int_e, Val (String ("hello", m), m), m)
...
let int_var =
  Local.BinOp (correct_binop_int_e, Plus m, Var (VarId ("foo", m), m), m)

let bool_var =
  Local.BinOp (Var (VarId ("foo", m), m), Or m, Val (Bool (false, m), m), m)
...
let correct_let_str_e =
  Local.Let
    (VarId ("foo", m), TString m, Val (String ("hello", m), m), string_var, m)
...
let local_pattn_typ_eq p expected_ctx expected_t =
  ...
  (local_typ_eq t expected_t
  && local_ctx_eq ctx expected_ctx
  && local_subst_eq subst [])
  |> assert_true

let const_suite =
  "Const type inference tests"
  >::: [
         ( "Correct infer local unit" >:: fun _ ->
           TUnit m |> local_expr_typ_eq correct_unit_e );
         ( "Correct infer local bool" >:: fun _ ->
           TBool m |> local_expr_typ_eq correct_binop_bool_e );
         ( "Correct infer local int" >:: fun _ ->
           TInt m |> local_expr_typ_eq correct_binop_int_e );
         ( "Correct infer local not bool" >:: fun _ ->
           TBool m |> local_expr_typ_eq correct_unop_bool_e );
         ( "Correct infer local and bool" >:: fun _ ->
           TBool m |> local_expr_typ_eq correct_and_bool_e );
         ( "Correct infer local neg int" >:: fun _ ->
           TInt m |> local_expr_typ_eq correct_unop_int_e );
         ( "Correct infer local pair" >:: fun _ ->
           TProd (TInt m, TString m, m) |> local_expr_typ_eq correct_pair_e );
         ( "Correct infer local fst pair" >:: fun _ ->
           TInt m |> local_expr_typ_eq correct_fst );
         ( "Correct infer local snd pair" >:: fun _ ->
           TString m |> local_expr_typ_eq correct_snd );
         ( "Correct infer local left sum" >:: fun _ ->
           TSum (TInt m, TVar (TypId ("T0", m), m), m)
           |> local_expr_typ_eq correct_left );
         ( "Correct infer local right sum" >:: fun _ ->
           TSum (TVar (TypId ("T0", m), m), TInt m, m)
           |> local_expr_typ_eq correct_right );
       ]

let local_binding_suite =
  "Local binding type inference tests"
  >::: [
         ( "Correct infer local int binding" >:: fun _ ->
           TInt m |> local_expr_typ_eq correct_let_int_e );
         ( "Correct infer local bool binding" >:: fun _ ->
           TBool m |> local_expr_typ_eq correct_let_bool_e );
         ( "Correct infer local string binding" >:: fun _ ->
           TProd (TInt m, TString m, m) |> local_expr_typ_eq correct_let_str_e
         );
         ( "Correct nested binding" >:: fun _ ->
           TProd (TInt m, TString m, m)
           |> local_expr_typ_eq correct_nested_binding );
       ]

let correct_pattn_suite =
  "Local pattern type inference tests"
  >::: [
         ( "Correct infer local int pattern" >:: fun _ ->
           TInt m |> local_pattn_typ_eq int_p [] );
         ( "Correct infer local default pattern" >:: fun _ ->
           TUnit m |> local_pattn_typ_eq def_p [] );
         ( "Correct infer local string pattern" >:: fun _ ->
           TString m |> local_pattn_typ_eq string_p [] );
         ( "Correct infer local pair pattern" >:: fun _ ->
           TProd (TVar (TypId ("T0", m), m), TString m, m)
           |> local_pattn_typ_eq pair_p [ ("foo", TVar (TypId ("T0", m), m)) ]
         );
         ( "Correct infer local left pattern" >:: fun _ ->
           TSum (TInt m, TVar (TypId ("T0", m), m), m)
           |> local_pattn_typ_eq left_int_p [] );
         ( "Correct infer local right pattern" >:: fun _ ->
           TSum (TVar (TypId ("T0", m), m), TUnit m, m)
           |> local_pattn_typ_eq right_def_p [] );
         ( "Correct pattern match" >:: fun _ ->
           TInt m |> local_expr_typ_eq int_pattn_match );
         ( "Correct local match with right-left order" >:: fun _ ->
           TInt m |> local_expr_typ_eq local_right_left_match );
       ]

let incorrect_local_type_suite =
  "Detect local type errors"
  >::: [
         ( "type error in local binop bool" >:: fun _ ->
           Failure "Unification failed"
           |> local_expr_typ_failures incorrect_binop_bool_e );
         ( "type error in local binop int" >:: fun _ ->
           Failure "Unification failed"
           |> local_expr_typ_failures incorrect_binop_int_e );
         ( "type error in local unop bool" >:: fun _ ->
           Failure "Unification failed"
           |> local_expr_typ_failures incorrect_unop_bool_e );
         ( "type error in local binop bool 2" >:: fun _ ->
           Failure "Unification failed"
           |> local_expr_typ_failures incorrect_and_bool_e );
         ( "type error in local unop int" >:: fun _ ->
           Failure "Unification failed"
           |> local_expr_typ_failures incorrect_unop_int_e );
         ( "Incorrect type binding" >:: fun _ ->
           Failure "Unification failed"
           |> local_expr_typ_failures incorrect_typ_binding );
         ( "Incorrect type annotation" >:: fun _ ->
           Failure "Type annotation and actual type mismatch"
           |> local_expr_typ_failures incorrect_typ_anno );
         ( "Unbound variable in local expression" >:: fun _ ->
           Failure "Variable not found when inferring expression"
           |> local_expr_typ_failures int_var );
         ( "Incorrect pattern match - return type mismatch" >:: fun _ ->
           Failure "Unification failed"
           |> local_expr_typ_failures mismatched_return_match );
         ( "Incorrect pattern match - pattern type mismatch" >:: fun _ ->
           Failure "Type of patterns are not sum types"
           |> local_expr_typ_failures mismatched_pattn_match );
         ( "Incorrect fst on non-product type" >:: fun _ ->
           let bad_fst = Local.Fst (Local.Val (Local.Int (1, m), m), m) in
           Failure "Fst Type error" |> local_expr_typ_failures bad_fst );
         ( "Incorrect snd on non-product type" >:: fun _ ->
...
let choreo_pattern_typ_eq p expected_ctx expected_t =
  ...
  (chreo_typ_eq t expected_t
  && choreo_ctx_eq ctx expected_ctx
  && choreo_subst_eq subst [])
  |> assert_true
...
let unify_local_failure t1 t2 expected_msg =
  ...
  with Failure msg -> assert_equal expected_msg msg
...
let correct_choreo_if =
  Choreo.If
    ( Choreo.LocExpr
        (Local.LocId ("Alice", m), Local.Val (Local.Bool (true, m), m), m),
      Choreo.LocExpr
        (Local.LocId ("Alice", m), Local.Val (Local.Int (1, m), m), m),
      Choreo.LocExpr
        (Local.LocId ("Alice", m), Local.Val (Local.Int (2, m), m), m),
      m )

let correct_choreo_fundef =
  Choreo.FunDef
    ([ Choreo.Var (Local.VarId ("foo", m), m) ], correct_choreo_loc_expr, m)
  ...
  Choreo.FunApp
    ( Choreo.FunDef
        ( [ Choreo.Var (Local.VarId ("x", m), m) ],
          ...
          m ),
      Choreo.LocExpr
        (Local.LocId ("Alice", m), Local.Val (Local.Int (0, m), m), m),
      m )
...
let correct_choreo_pair =
  Choreo.Pair (correct_choreo_loc_expr, correct_choreo_send, m)
...
let correct_left =
  Choreo.Left
    ( Choreo.LocExpr
        (Local.LocId ("Alice", m), Local.Val (Local.Int (5, m), m), m),
      m )
...
let correct_choreo_let_bool_e =
  Choreo.Let
    ( [
      ...
      Choreo.LocExpr
        (Local.LocId ("Bob", m), Local.Val (Local.Bool (true, m), m), m),
      m )
...
let correct_choreo_let_str_e =
  Choreo.Let
    ( [
      ...
      Choreo.Pair
        ( Choreo.LocExpr
            (Local.LocId ("Charlie", m), Local.Val (Local.Int (1, m), m), m),
          Choreo.LocExpr
            ( Local.LocId ("Charlie", m),
              Local.Val (Local.String ("hello", m), m),
              m ),
          m ),
      m )
...
let choreo_pair_p : ftv Choreo.pattern =
  Choreo.Pair (choreo_var_p, choreo_loc_int_p, m)
...
let choreo_mismatched_return_match =
  Choreo.Match
    ( Choreo.Var (Local.VarId ("foo", m), m),
      [
        (choreo_left_loc_p, correct_choreo_loc_expr);
        (choreo_right_def_p, Choreo.Unit m);
      ],
      m )
...
let choreo_const_suite =
  ...
  >::: [
         ( "Correct infer choreo unit" >:: fun _ ->
           Choreo.TUnit m |> choreo_expr_typ_eq correct_choreo_unit_e );
         ( "Correct infer location expression" >:: fun _ ->
           Choreo.TLoc (Local.LocId ("Alice", m), Local.TInt m, m)
           |> choreo_expr_typ_eq correct_choreo_loc_expr );
         ( "Correct infer send" >:: fun _ ->
           Choreo.TLoc (Local.LocId ("Bob", m), Local.TInt m, m)
           |> choreo_expr_typ_eq correct_choreo_send );
         ( "Correct infer if" >:: fun _ ->
           Choreo.TLoc (Local.LocId ("Alice", m), Local.TInt m, m)
...
let correct_choreo_pattern_suite =
  ...
  >::: [
         ( "Correct infer default pattern" >:: fun _ ->
           Choreo.TUnit m |> choreo_pattern_typ_eq choreo_def_p [] );
         ( "Correct infer var pattern" >:: fun _ ->
           Choreo.TVar (Choreo.Typ_Id ("T0", m), m)
           |> choreo_pattern_typ_eq choreo_var_p
                [ ("foo", Choreo.TVar (Choreo.Typ_Id ("T0", m), m)) ] );
         ( "Correct infer location int pattern" >:: fun _ ->
           Choreo.TLoc (Local.LocId ("Alice", m), Local.TInt m, m)
           |> choreo_pattern_typ_eq choreo_loc_int_p [] );
         ( "Correct infer pair pattern" >:: fun _ ->
           Choreo.TProd
             ( Choreo.TVar (Choreo.Typ_Id ("T0", m), m),
               Choreo.TLoc (Local.LocId ("Alice", m), Local.TInt m, m),
               m )
           |> choreo_pattern_typ_eq choreo_pair_p
                [ ("foo", Choreo.TVar (Choreo.Typ_Id ("T0", m), m)) ] );
         ( "Correct infer left pattern" >:: fun _ ->
           Choreo.TSum
             ( Choreo.TLoc (Local.LocId ("Alice", m), Local.TInt m, m),
               Choreo.TVar (Choreo.Typ_Id ("T0", m), m),
               m )
         ...
         ( "Correct left pattern with non-TLoc type" >:: fun _ ->
           Choreo.TSum
             ( Choreo.TLoc
                 ( Local.LocId ("dummy", m),
                   Local.TVar (Local.TypId ("T0", m), m),
                   m ),
               Choreo.TVar (Choreo.Typ_Id ("T1", m), m),
               m )
           |> choreo_pattern_typ_eq choreo_left_def_p [] );
         ( "Correct right pattern with TLoc type" >:: fun _ ->
           Choreo.TSum
             ( Choreo.TVar (Choreo.Typ_Id ("T0", m), m),
               Choreo.TLoc (Local.LocId ("Alice", m), Local.TInt m, m),
               m )
           |> choreo_pattern_typ_eq choreo_right_loc_p [] );
         ( "Correct choreo match with TLoc patterns" >:: fun _ ->
           Choreo.TLoc (Local.LocId ("Alice", m), Local.TInt m, m)
...
let incorrect_choreo_type_suite =
  ...
  >::: [
                 ...
                 Choreo.Let
                   ...
                   ( [
                       Choreo.Decl
                         ( Choreo.Var (Local.VarId ("x", m), m),
                           Choreo.TLoc
                             (Local.LocId ("Alice", m), Local.TString m, m),
                           m );
                     ],
                     Choreo.LocExpr
                       ( Local.LocId ("Alice", m),
                         Local.Val (Local.Int (1, m), m)
         ...
         ( "Let with multiple declarations" >:: fun _ ->
           let multi_decl_let =
             Choreo.Let
               ( [
                   Choreo.Decl
...
(** 
These are all the argument types a variant constructor can take. 
We want to write tests that include all of them. 
...
(**
These are some example constructors that the variants can have.
For the purposes of testing simplicity, they all have the same return type so that they can be used in the same variant together.
*)
...
let local_constructor_10_args : ftv Local.constructor = {
  ...
  args = [local_argunit; local_argunit; local_argunit; local_argunit; local_argunit; 
          local_argunit; local_argunit; local_argunit; local_argunit; local_argunit]; 
  typ = Local.TypId( "VariantType", m ); 
  info = m
}
...
let local_constructor_mixed_args : ftv Local.constructor = {
  ...
  args = [local_argint; local_argstring; local_argbool; local_argvar; local_argprod; local_argsum; local_argvariant]; 
  typ = Local.TypId( "VariantType", m ); 
  info = m
}

let correct_local_variant_simple1 = Local.TVariant([local_no_args_constructor], m)
let correct_local_variant_simple2 = Local.TVariant([local_simple_constructor], m)
...
let unify_local_variant_tests = 
  ...
  >::: [
        ...
        ( "simple test 1" >:: fun _ ->
        unify_local_success correct_local_variant_simple1 correct_local_variant_simple1 []);
        ( "simple test 2" >:: fun _ ->
        unify_local_success correct_local_variant_simple2 correct_local_variant_simple2 []);
        ( "simple test 3" >:: fun _ ->
        unify_local_success correct_local_variant_big_constructor correct_local_variant_big_constructor []);
        ( "simple test 4" >:: fun _ ->
        unify_local_success correct_local_variant_mixed correct_local_variant_mixed []);
       
        (*unify_local bad inputs tests*)
        ( "constructor amount mismatch: 1 and 3" >:: fun _ ->
        unify_local_failure correct_local_variant_simple1 correct_local_variant_3constructs "Local variant constructor count mismatch");
        ...
        let a = Local.TVariant([local_no_args_constructor; local_simple_constructor], m) in
        let b = Local.TVariant([local_simple_constructor; local_no_args_constructor], m) in
        unify_local_failure a b "Local variant constructor name mismatch" );
        ( "constructor argument amount mismatch: 0 and 1" >:: fun _ ->
        unify_local_failure correct_local_variant_simple1 correct_local_variant_simple2 "Local variant constructor arg count mismatch");
       ]

(**
There are the two substitutions for the apply_subst test suite.
1) make no substitutions (verify that the input is unchanged)
2) substitute arguments of type "A" for unit (verify that it changes)
*)
let subst_nothing = [] 
...
let local_ab_constructor_subst : ftv Local.constructor = {
  name = Local.TypId("Name", m); 
  args = [Local.TUnit m; local_var_b]; (*since we're only substituting for A, B should remain unsubstituted*)
  typ = Local.TypId( "VariantType", m ); 
  info = m
}
...
let local_variant_substitution_tests =
  ...
  >::: [
      ( "variant with no substitutions" >:: fun _ ->
        let subsituted_variant = apply_subst_typ_local subst_nothing correct_local_variant_simple1 in
        assert_equal subsituted_variant correct_local_variant_simple1 );
      ( "substitute 'A' for unit (single argument constructor)" >:: fun _ ->
        let subsituted_variant = apply_subst_typ_local subst_a_unit local_variant_a in
        assert_equal subsituted_variant local_variant_a_subst );
      ( "substitute 'A' for unit (mixed arg constructor)" >:: fun _ ->
        let subsituted_variant = apply_subst_typ_local subst_a_unit local_variant_mixed_vars in
...
let choreo_constructor_10_args : ftv Choreo.constructor = {
  ...
  args = [choreo_argunit; choreo_argunit; choreo_argunit; choreo_argunit; choreo_argunit; 
          choreo_argunit; choreo_argunit; choreo_argunit; choreo_argunit; choreo_argunit]; 
  typ = Local.TypId( "VariantType", m ); 
  info = m
}
let choreo_constructor_mixed_args : ftv Choreo.constructor = {
  name = Local.TypId("mixed_args", m); 
  args = [choreo_argunit; choreo_argloc; choreo_argvar; choreo_argmap; choreo_argprod; choreo_argsum; choreo_argvariant]; 
  typ = Local.TypId( "VariantType", m ); 
  info = m
}
(*
...
let correct_choreo_variant_simple1 = Choreo.TVariant([choreo_no_args_constructor], m)
let correct_choreo_variant_simple2 = Choreo.TVariant([choreo_simple_constructor], m)
(*changed*)
...
let unify_choreo_variant_tests = 
  ...
  >::: [
        ...
        ( "simple test 1" >:: fun _ ->
        unify_choreo_success correct_choreo_variant_simple1 correct_choreo_variant_simple1 []);
        ( "simple test 2" >:: fun _ ->
        unify_choreo_success correct_choreo_variant_simple2 correct_choreo_variant_simple2 []);
        ( "simple test 3" >:: fun _ ->
        unify_choreo_success correct_choreo_variant_big_constructor correct_choreo_variant_big_constructor []);
        ( "simple test 4" >:: fun _ ->
        unify_choreo_success correct_choreo_variant_mixed correct_choreo_variant_mixed []);
       
        (*unify_choreo bad inputs tests*)
        ( "constructor amount mismatch: 1 and 3" >:: fun _ ->
        unify_choreo_failure correct_choreo_variant_simple1 correct_choreo_variant_3constructs "Choreo variant constructor count mismatch");
        ( "constructor name mismatch: 'a' and 'b'" >:: fun _ ->
        ...
        ( "constructor order mismatch: 'a' and 'b'" >:: fun _ ->
        let a = Choreo.TVariant([choreo_no_args_constructor; choreo_simple_constructor], m) in
        let b = Choreo.TVariant([choreo_simple_constructor; choreo_no_args_constructor], m) in
        unify_choreo_failure a b "Choreo variant constructor name mismatch" );
        ( "constructor argument amount mismatch: 0 and 1" >:: fun _ ->
        unify_choreo_failure correct_choreo_variant_simple1 correct_choreo_variant_simple2 "Choreo variant constructor arg count mismatch");
       ]
...
let choreo_ab_constructor_subst : ftv Choreo.constructor = {
  name = Local.TypId("Name", m); 
  args = [Choreo.TUnit m; choreo_var_b]; (*since we're only substituting for A, B should remain unsubstituted*)
  typ = Local.TypId( "VariantType", m ); 
  info = m
}

let choreo_variant_a = Choreo.TVariant([choreo_1var_constructor;], m)
let choreo_variant_a_subst = Choreo.TVariant([choreo_1var_constructor_subst;], m)
...
let choreo_variant_substitution_tests =
  ...
  >::: [
      ( "variant with no substitutions" >:: fun _ ->
        let subsituted_variant = apply_subst_typ_choreo subst_nothing correct_choreo_variant_simple1 in
        assert_equal subsituted_variant correct_choreo_variant_simple1 );
      ( "substitute 'A' for unit (single argument constructor)" >:: fun _ ->
        let subsituted_variant = apply_subst_typ_choreo subst_a_unit choreo_variant_a in
        assert_equal subsituted_variant choreo_variant_a_subst );
      ( "substitute 'A' for unit (mixed arg constructor)" >:: fun _ ->
        let subsituted_variant = apply_subst_typ_choreo subst_a_unit choreo_variant_mixed_vars in
...
let unification_suite =
  ...
  >::: [
         ( "Correct unify_local string" >:: fun _ ->
           unify_local_success (Local.TString m) (Local.TString m) [] );
         ( "Correct unify_local unit" >:: fun _ ->
           unify_local_success (Local.TUnit m) (Local.TUnit m) [] );
         ( "Correct unify_local var" >:: fun _ ->
           unify_local_success
             (Local.TVar (Local.TypId ("X", m), m))
             (Local.TInt m)
             [ ("X", Local.TInt m) ] );
         ( "Incorrect unify_local var occurs check" >:: fun _ ->
           let tvar = Local.TVar (Local.TypId ("X", m), m) in
           unify_local_failure tvar
             (Local.TProd (tvar, Local.TUnit m, m))
             "Occurs check failed" );
         ( "Correct unify_local var right side" >:: fun _ ->
           let tvar = Local.TVar (Local.TypId ("X", m), m) in
           unify_local_success (Local.TInt m) tvar [ ("X", Local.TInt m) ] );
         ( "Correct unify_local same var" >:: fun _ ->
           let tvar = Local.TVar (Local.TypId ("X", m), m) in
           unify_local_success tvar tvar [] );
         ( "Correct unify_local prod" >:: fun _ ->
           unify_local_success
             (Local.TProd (Local.TInt m, Local.TBool m, m))
             (Local.TProd (Local.TInt m, Local.TBool m, m))
             [] );
         ( "Correct unify_local sum" >:: fun _ ->
           unify_local_success
             (Local.TSum (Local.TInt m, Local.TBool m, m))
             (Local.TSum (Local.TInt m, Local.TBool m, m))
             [] );
         ( "Incorrect unify_local prod mismatch" >:: fun _ ->
           unify_local_failure
             (Local.TProd (Local.TInt m, Local.TBool m, m))
             (Local.TProd (Local.TBool m, Local.TBool m, m))
             "Unification failed" );
         ( "Correct unify_choreo unit" >:: fun _ ->
           unify_choreo_success (Choreo.TUnit m) (Choreo.TUnit m) [] );
         ( "Incorrect unify_choreo loc" >:: fun _ ->
           unify_choreo_failure
             (Choreo.TLoc (Local.LocId ("Alice", m), Local.TInt m, m))
             (Choreo.TLoc (Local.LocId ("Bob", m), Local.TInt m, m))
             "Location mismatch" );
         ( "Incorrect unify_choreo var" >:: fun _ ->
           let tvar = Choreo.TVar (Choreo.Typ_Id ("X", m), m) in
           unify_choreo_failure tvar
             (Choreo.TProd (tvar, Choreo.TUnit m, m))
             "Occurs check failed" );
         ( "Correct unify_choreo var right side" >:: fun _ ->
           let tvar = Choreo.TVar (Choreo.Typ_Id ("X", m), m) in
           unify_choreo_success (Choreo.TUnit m) tvar [ ("X", Choreo.TUnit m) ]
         );
         ( "Correct unify_choreo same var" >:: fun _ ->
           let tvar = Choreo.TVar (Choreo.Typ_Id ("X", m), m) in
         ...
         ( "Correct unify_choreo prod" >:: fun _ ->
           unify_choreo_success
             ...
             (Choreo.TProd (Choreo.TUnit m, Choreo.TUnit m, m))
             [] );
         ( "Correct unify_choreo sum" >:: fun _ ->
           unify_choreo_success
  ...
  >::: [
         ...
         ( "get_choreo_ctx test" >:: fun _ ->
           ...
           assert_equal
             ...
             result );
         ( "get_local_subst non-TLoc test" >:: fun _ ->
           let choreo_subst =
             [
               ("x", Choreo.TUnit m);
         ...
         ( "apply local substitution to TVar" >:: fun _ ->
           let tvar = Local.TVar (Local.TypId ("X", m), m) in
         ...
         ( "apply choreo substitution to TVar" >:: fun _ ->
           let tvar = Choreo.TVar (Choreo.Typ_Id ("X", m), m) in
