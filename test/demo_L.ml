...
let () =
  ...
  Send_receive.init_http_server "L" ();
  (let process_L =
     ...
     let rec main_PIROUETTE_ID =
       let rec idle_PIROUETTE_ID = print_idle_PIROUETTE_ID "[L] idle" in
       let rec nap_PIROUETTE_ID = sleep_PIROUETTE_ID 1 in
       let rec waiting_PIROUETTE_ID =
         print_waiting_PIROUETTE_ID "L waiting on E..." in
       let rec inbox_PIROUETTE_ID =
         Marshal.from_string (Send_receive.receive_message ~location:"L") 0 in
       let rec nap_PIROUETTE_ID = sleep_PIROUETTE_ID () in
       let rec received_PIROUETTE_ID =
         print_done_PIROUETTE_ID "L received from E!" in
       let rec nap_PIROUETTE_ID = sleep_PIROUETTE_ID 1 in
       let rec sending_PIROUETTE_ID =
         print_sending_PIROUETTE_ID "L sending to H..." in
       let rec _unit_25 =
         let val_24 = inbox_PIROUETTE_ID in
         match Lwt_main.run
                 (Send_receive.send_message ~location:"H"
                    ~data:(Marshal.to_string val_24 []))
         with
         | Ok () -> ()
         | Error msg -> failwith ("Send error: " ^ msg) in
       let rec nap_PIROUETTE_ID = sleep_PIROUETTE_ID 1 in
       let rec done_PIROUETTE_ID =
         print_done_PIROUETTE_ID "L send to H sucessful!" in
       () in
     () in
   ignore process_L)