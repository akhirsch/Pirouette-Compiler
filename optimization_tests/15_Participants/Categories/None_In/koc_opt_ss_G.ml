...
let () =
  ...
  let process_G =
    ...
    let rec broadcast_opt freq =
      match
        match Lwt_main.run (Send_receive.receive_message ~location:"G") with
        | Ok msg -> msg
        | Error msg -> failwith ("Receive error: " ^ msg)
      with
      | "L" ->
        (match Lwt_main.run (Send_receive.send_message ~location:"N" ~data:"L") with
         | Ok () -> ()
         | Error msg -> failwith ("Send error: " ^ msg));
        (match Lwt_main.run (Send_receive.send_message ~location:"O" ~data:"L") with
         | Ok () -> ()
         | Error msg -> failwith ("Send error: " ^ msg));
        let rec x = 10 in
        broadcast_opt ()
      | "R" ->
        (match Lwt_main.run (Send_receive.send_message ~location:"N" ~data:"R") with
         | Ok () -> ()
         | Error msg -> failwith ("Send error: " ^ msg));
        (match Lwt_main.run (Send_receive.send_message ~location:"O" ~data:"R") with
         | Ok () -> ()
         | Error msg -> failwith ("Send error: " ^ msg));
        let rec x = 9 in
        ()
    ...
    in
    let rec _unit_7 = broadcast_opt () in
    ()
  in
  ignore process_G
;;
