...
let () =
  ...
  let process_D =
    ...
    let rec broadcast_opt freq =
      match
        match Lwt_main.run (Send_receive.receive_message ~location:"D") with
        | Ok msg -> msg
        | Error msg -> failwith ("Receive error: " ^ msg)
      with
      | "L" ->
        (match Lwt_main.run (Send_receive.send_message ~location:"H" ~data:"L") with
         | Ok () -> ()
         | Error msg -> failwith ("Send error: " ^ msg));
        (match Lwt_main.run (Send_receive.send_message ~location:"I" ~data:"L") with
         | Ok () -> ()
         | Error msg -> failwith ("Send error: " ^ msg));
        let rec x = 10 in
        broadcast_opt ()
      | "R" ->
        (match Lwt_main.run (Send_receive.send_message ~location:"H" ~data:"R") with
         | Ok () -> ()
         | Error msg -> failwith ("Send error: " ^ msg));
        (match Lwt_main.run (Send_receive.send_message ~location:"I" ~data:"R") with
         | Ok () -> ()
         | Error msg -> failwith ("Send error: " ^ msg));
        let rec x = 9 in
        ()
    ...
    in
    let rec _unit_4 = broadcast_opt () in
    ()
  in
  ignore process_D
;;
