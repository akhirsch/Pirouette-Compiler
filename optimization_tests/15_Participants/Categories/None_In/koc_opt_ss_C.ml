...
let () =
  ...
  let process_C =
    ...
    let rec broadcast_opt freq =
      ...
      with
      | "L" ->
        (match Lwt_main.run (Send_receive.send_message ~location:"F" ~data:"L") with
         | Ok () -> ()
         | Error msg -> failwith ("Send error: " ^ msg));
        (match Lwt_main.run (Send_receive.send_message ~location:"G" ~data:"L") with
         | Ok () -> ()
         | Error msg -> failwith ("Send error: " ^ msg));
        let rec x = 10 in
        broadcast_opt ()
      | "R" ->
        (match Lwt_main.run (Send_receive.send_message ~location:"F" ~data:"R") with
         | Ok () -> ()
         | Error msg -> failwith ("Send error: " ^ msg));
        (match Lwt_main.run (Send_receive.send_message ~location:"G" ~data:"R") with
         | Ok () -> ()
         | Error msg -> failwith ("Send error: " ^ msg));
        let rec x = 9 in
        ()
